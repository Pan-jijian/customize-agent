#!/bin/bash
# 九度实测编排（4.8.3：Final Gate 补写三段式要求+标签规范化扩展+链尾空壳兜底）：
# 杀旧进程 → 重启 dev server 加载 4.8.3 → 提交任务 → 挂轮询
set -u
LOG="/tmp/round9-orchestrate.log"
...
nohup pnpm dev > /tmp/round9-server.log 2>&1 &
...
echo "round9 doc=$DOC_ID $(date +%H:%M:%S)" >> "$LOG"
chmod +x /Users/pan/Desktop/codeing/customize-agent/.dbg/selftest-poll-round9.sh
nohup /Users/pan/Desktop/codeing/customize-agent/.dbg/selftest-poll-round9.sh "$DOC_ID" > /dev/null 2>&1 &
