#!/bin/bash
# 十度实测编排（4.8.4：畸形标签归一化+comparable短串防护+链回退流程段+参数口径补全）：
# 杀旧进程 → 重启 dev server 加载 4.8.4 → 提交任务 → 挂轮询
set -u
LOG="/tmp/round10-orchestrate.log"
...
cd /Users/pan/Desktop/codeing/customize-agent/apps/server
export PATH="/Users/pan/.nvm/versions/node/v26.4.0/bin:$PATH"
nohup pnpm dev > /tmp/round10-server.log 2>&1 &
...
echo "round10 doc=$DOC_ID $(date +%H:%M:%S)" >> "$LOG"
chmod +x /Users/pan/Desktop/codeing/customize-agent/.dbg/selftest-poll-round10.sh
nohup /Users/pan/Desktop/codeing/customize-agent/.dbg/selftest-poll-round10.sh "$DOC_ID" > /dev/null 2>&1 &
