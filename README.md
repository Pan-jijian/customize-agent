...
├── tools/      文件读写、工作区快照、安全路径、沙箱、终端、Git、Diff、语法验证、多媒体/导出工具
├── search/     tree-sitter 索引 + ripgrep 搜索 + LSP + 语义 Embedding
├── engine/     ToolRegistry、权限、执行控制、上下文、规划、子智能体、MCP、Hooks
├── runtime/    ConfigStore + ModelRegistry + 可复用运行时封装 + 遥测 + 生命周期
├── memory/     跨会话记忆 (SQLite + FTS5)
└── apps/cli/   Commander CLI 入口、工具注册装配、REPL、TUI 渲染/输入、i18n、像素字
```
...
types (零依赖)
  ↑
  ├── llm → OpenAI/Anthropic/Google SDK
  ├── search → tree-sitter, better-sqlite3, vscode-languageserver-protocol
  ├── tools → search, sandbox/process/media/export 相关依赖
  ├── engine → llm, types
  ├── runtime → llm, engine, types
  ├── memory → better-sqlite3
  └── apps/cli → engine, llm, tools, search, runtime, memory
```
...
| `BuiltinTools` | CLI 内置工具能力集合：文件、网络、Git、进程、导出、多媒体、MCP/Plugin 配置 |
| `WorkspaceFs` | 工作区安全路径解析与基础文件操作 |
| `WorkspaceSnapshotService` | 工作区快照/检查点创建、恢复、删除 |
...
| 导出 | 说明 |
|------|------|
| `AgentRuntime` | 可复用运行时封装：组件生命周期 + 会话 + 主循环 + 取消传播；CLI 主链路当前直接装配 AgentExecutor |
