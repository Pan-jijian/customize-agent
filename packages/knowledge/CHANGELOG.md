...
- 修复：大幅面图纸 PDF 索引时 OOM 崩溃（531 页 / 183MB 实测）

  ## 现象

  丰乐镇项目全量重新索引时，worker 反复 `SIGABRT`，**整个索引任务失败且库被删到一半**
  （实测 RSS 逐秒爬升 3.0 → 7.45 → 8.92GB 后崩溃；`v8::internal::V8::FatalProcessOutOfMemory`）。

  ## 根因（两处写死的上限把「本可完成」变成「硬崩溃」）

  1. **`tryRenderWithPyMuPDF` 的 60 秒超时**：531 页 / 183MB 的施工图 PDF，PyMuPDF 渲染实测需 **135 秒**。
     60 秒超时把**成功路径掐断**，于是降级到进程内的 pdfjs 逐页 4× 渲染 —— 在 Node 堆里直接吃穿。
  2. **逐页不释放**：`extractPdfText` 对 531 页逐页调用 `getTextContent`，而
     `pdfPageHasDrawableContent` 会读 `page.getOperatorList()`；CAD 矢量图纸单页算子表可达数十万条，
     全文档累计后 pdfjs 缓存把堆吃穿（**进程启动后 2 秒内即 OOM**）。

  ## 修复

  - **超时按文件规模自适应**：`min(30min, max(60s, sizeMb × 3000))` —— 写死的超时不该把可完成的任务判死。
  - **逐页显式释放**：`extractPdfText` 每页处理完调用 `page.cleanup()`，内存回到「单页量级」而非「全文档累计」。
  - **渲染倍率按像素上限自适应**：`renderScale` 取「默认 4×」与「单页不超 40MP」的较小值、下限 1×
    （大幅面页自动降倍率、小页面仍享 4× 清晰度，**不是一刀切降精度**）。并逐页 `cleanup()` + 释放 canvas。
  - **进程内渲染显式拒绝而非崩溃**：pdfjs 降级路径对 >120 页的文档直接返回 null 并告警
    （「渲染不可用」比「整个索引任务崩掉、库被删一半」好得多）。
  - **索引 worker 堆按机器定量**：`min(12GB, max(4GB, 物理内存/4))`，`KB_INDEX_MAX_OLD_SPACE_MB` 可覆盖
    （worker 与主服务分进程，不影响主进程）。

  ## 实测验证

  同一份 531 页 PDF：`extractPdfText` **完整跑完**，峰值从 **>8GB（崩溃）降至 3.1GB**；
  随后 OCR 阶段稳定在 2.2–2.4GB、CPU 满载持续推进，**不再 OOM**。

- 提速：ONNX session 线程分档（**1.3×，逐切片识别文本逐字节一致**）

  ## 现象与归因

  531 页图纸 PDF 的解析在 OCR 段约 87 分钟（16,148 个切片 × 322ms）。拆解单切片墙钟：
  **推理 81%**（检测 23% / 识别 50% / 方向分类 8%）+ **JS 主线程 19%**，裁剪与后处理 <1%。

  三个 session 都走 ORT 默认 intra-op 线程数（≈ 物理核数），实测**进程占用 7.4 核但大量时间消耗在
  线程自旋而非有效计算**：把线程数调小反而更快，说明默认档位对这三个体量差异极大的模型是错的 ——
  方向分类是 1MB 的小分类模型，给满线程纯属浪费。

  ## 处置

  `createService` 里本就有的 `ort` 注入点包一层：按**模型 ArrayBuffer 引用相等**识别 det/rec/ori
  三个 session（库的 `initialize()` 原样透传我们传入的 buffer，故换模型代次/preset 都不会失配），
  分别注入 `intraOpNumThreads`，默认 `det=4 / rec=4 / ori=1`。
  `CUSTOMIZE_KB_OCR_THREADS="det=4,rec=4,ori=1"` 可覆盖，`off` 关闭分档（排障与 A/B 对照）。

  注入失败时**回退未包装 ort 重建**：provider 的兜底是降级 tesseract.js（识别质量明显更差），
  一个提速优化不该把主引擎弄丢。

  ## 实测（10 核 macOS，A2 图纸 200DPI 切片，真实 provider 路径，四轮交错复测）

  | 配置       | 每切片                      | 进程占用 |
  | ---------- | --------------------------- | -------- |
  | 默认线程   | 304–317ms                   | 7.2 核   |
  | 分档 4/4/1 | 232–244ms（**1.30–1.37×**） | 3.9 核   |

  四轮**逐切片识别文本逐字节一致**（无损）。全文档预期 OCR 段 87 → 约 70 分钟。

  ## 已实测否决（不再重复投入）

  - **多实例并行**（2/3/4/5 路独立 session，绕开 provider 串行锁）：0.78–1.16×，线程超订只会更慢。
  - **CoreML EP**（macOS GPU/ANE）：**慢 1.68×**，且检测框数 164→183、**输出不一致**（非无损）。
  - 整体调小线程数（6 线程）：更慢；关闭方向分类器：rec 快 45% 但**文本改变**（损质量，排除）；
    跳过空白切片：图纸每片都有墨迹（0 个空白片）；跳过重复页：531 页零重复。

  > 测量口径提醒：以上结论均在**空载**下测得。此前几轮「多 session 无收益」的数据是在后台
  > `check` 门禁抢 CPU 时采集的（单切片 930ms vs 空载 322ms，差 3 倍），已作废。
...
- CAD 乱码判定收窄：消除剩余误杀（汉字自然重复 + 数字编号）

  继上一版移除「整句中文判乱码」规则后，对巢湖 21 张图纸全量复核剩余误杀，定位到最后两条
  误杀规则并逐一收窄。**同批语料误杀由 32 处降至 0 处**（初版为 873 处）：
  ...
  ## 一、移除「短行内同一汉字高频重复 → 乱码」（14 字符内重复汉字占比 >= 60%）

  中文本身就自然重复用字，该判据把大量正常标注误杀：「电动伸缩门或电动移门」
  「一层平面图、屋顶平面图」「阀门(带阀门井)」「（地砖踢脚踢脚）」「分层压实，素土回填分层压实」
  「规定性指标 性能性指标」。实测命中 22 处（去重 11 条）**全部是正常中文标注、无一条真乱码**。

  替换为按「字符表塌缩」判定：`不同字符数 / 行长度 < 0.6`（6~14 字符行）。循环乱码
  （摁䭚摁譚摁譚摁：7 字只有 3 个不同字）字符表会塌缩到六成以下，而正常中文不会——
  「电动伸缩门或电动移门」8/9、「阀门(带阀门井)」6/8、「规定性指标性能性指标」7/10。
  数字占比 >= 30% 的行豁免，避免 `800*800*800`、`1000*1000*800/450` 这类重复尺寸数字被误伤。

  该规则与既有的「汉字-Latin-汉字交叉混排」互补而非冗余：后者管交叉混排，前者管同表循环
  （摁䭚摁譚摁譚摁 无 Latin，且生僻字占比 1/7 低于「CJK 扩展区生僻字」的 0.2 门槛，两者都兜不住）。

  ## 二、收窄「行内短片段周期性重复 → 乱码」

  原判据的分母是「短片段自身的总长度」，极小：含条款编号的行（`4.4.2`、`12.2.2`、`2.3.3`、
  `7.1.5.1`）里重复的**数字**就会在 3 个字符的分母上触发 >= 0.4，实测 8 条正常中文条款被误杀。
  同时把大量真实尺寸/材料标注一并误杀：`800*800*800`、`1000*1000*800/450`、`700*800*800/450`、
  `20*20*0.7`。

  收窄为两个必要条件，缺一不可：

  - **仅限无汉字行**——真循环模式是 Latin/符号串；含汉字行里的编号不再参与判定。
  - **重复片段不能是纯数字**——循环数字是尺寸标注本身；二进制误读吐的是高熵混合串，
    不会只产生纯数字循环。

  真乱码循环模式（`Ml+Ml+Ml`、`AM~AM~BM~BM`、`2dA+2dA+2dA`）全部仍被拦下。

  ## 三、测试

  `cad-garbled-detector.test.ts` 扩至 4 条：21 条实测被误杀的正常标注（含设备名/材料名/
  制度说明/条款编号/公司名）全部保留；4 条循环尺寸标注保留；5 条真乱码循环模式仍被拦下；
  Latin-1 扩展字符密集行仍被拦下。
...
- 4.44.0 门禁链源头根治（丰乐镇 4.43 实测 45 项阻断全量定因 + 全链落码）：
  ...
  - 验证：pnpm run check（turbo typecheck lint + vitest 全量 12899 用例）全绿
...
- 修复 PDF 空文本层图形页（扫描图/CAD 矢量图页）整页内容丢失：空文本层页若存在绘制内容（图片/矢量描边）则进入选择性 OCR 候选兜底；切分器对 "## PDF 第 N 页（OCR）" 强标题节强制独立，防止 section_title 错挂到前一节标题。
...
- 修复 PDF 提取中 OCR 图形噪声导致的伪乱码文本入库问题：

  - 新增 `filterOcrGraphicNoiseLines`：过滤 LOGO、图框线等纯图形区域被 OCR 误识别出的符号混排噪声行（如 `< ] 人人 AL 已忌 | | 三 | ra | \ 几 L AN`），同时保护正常工程标注行（编号行、领域信号行、工程符号行）
  - 乱码页选择性 OCR 增加页级最低质量门槛（汉字数、文本长度、有效行占比三重判定），纯图形页 OCR 噪声碎片整页丢弃
  - PyMuPDF 渲染增加 python 解释器自动探测（`findPyMuPDFInterpreter`），服务器 PATH 中 python3 无 fitz 时自动回退到系统解释器，避免静默降级到 pdfjs 低质量渲染导致 OCR 出碎片
