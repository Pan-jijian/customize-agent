...
// 4.40.0 零商务句口径：商务数据清洗与检测均无豁免——正文任何含商务词/税率的句一律删除/上报。
// 「定性响应句豁免」通道（isSanctionedResponseSentence）随商务全链清理删除：商务域条款在判定层
// 即排除出要求池，正文不产出也不豁免任何商务声明句。
describe('E4 stripCommercialDataBodyLines 商务句零豁免清洗', () => {
  it('E4 含商务词的定性句（含「按合同约定」式声明）→ 一并删除', () => {
    const md = '按招标文件约定：本工程暂列金额按招标文件约定计入投标总价并按规定计税，暂列金额的使用范围与计价规则按合同约定执行。';
    const result = stripCommercialDataBodyLines(md);
    expect(result).not.toContain('暂列金额');
  });
  it('E4 「按招标文件要求：」条幅含商务数字 → 删除', () => {
    expect(stripCommercialDataBodyLines('按招标文件要求：暂列金额为10万元。')).not.toContain('暂列金额');
  });
  it('E4 「按招标文件要求：」条幅无商务数字 → 同样删除（零豁免）', () => {
    expect(stripCommercialDataBodyLines('按招标文件要求：暂列金额的使用按约定管理。')).not.toContain('暂列金额');
  });
  it('E4 非受控污染句仍删（防御全线不放松）', () => {
    const result = stripCommercialDataBodyLines('本项目暂列金额为60万元。保留句。');
    expect(result).not.toContain('暂列金额');
    expect(result).toContain('保留句。');
  });
  it('E4 超 120 字受控句仍删（窗口无关，零豁免）', () => {
