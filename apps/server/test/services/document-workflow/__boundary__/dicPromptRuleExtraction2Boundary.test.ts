...
describe('buildRuntimePromptRules', () => {
  ...
  it('chapterRules 匹配', () => {
    const r = buildRuntimePromptRules(...[{"promptTexts":"施工方案章节必须包含工艺流程；质量保证章节禁止套话","template":{"chapters":[{"title":"施工方案"},{"title":"质量保证措施"}]}}]);
  ...
  it('chapterRules 无匹配过滤', () => {
    const r = buildRuntimePromptRules(...[{"promptTexts":"必须输出封面","template":{"chapters":[{"title":"施工方案"}]}}]);
...
describe('extractPromptStructuralRules', () => {
  ...
  it('chapters 映射标题', () => {
    const r = extractPromptStructuralRules(...["第一章必须小节：1.编制说明",[{"title":"编制说明与工程概况"}]]);
...
describe('runtimePromptRulesPrompt', () => {
  ...
  it('商务敏感', () => {
    const r = runtimePromptRulesPrompt(...[{"coverPolicy":"unspecified","tocPolicy":"unspecified","forbidCover":false,"forbidToc":false,"forbiddenTerms":["知识库","提示词","建议补充","资料库","OCR","后台话术","后台流程","后台数据","后台资料","后台溯源","绑定片段","报价明细表"],"preferredTerms":[{"from":"高度重视","to":"严格落实"},{"from":"重中之重","to":"关键控制事项"}],"requiredTables":[],"requiredKeywords":[],"forbiddenPatterns":[],"sourceHash":"527004e","exactHeadings":[],"forbidExtraHeadings":false,"requiredSubjects":[],"forbiddenSubjects":[],"backendTerms":["知识库","提示词","建议补充","资料库","OCR","后台话术","后台流程","后台数据","后台资料","后台溯源","绑定片段"],"commercialTerms":["报价明细表"],"forbidFabrication":false,"requireEvidenceForQuantities":false,"preferProjectFacts":false,"chapterRules":[],"roleRules":[],"executionSummary":["已识别禁用词 12 个"]}]);
