...
describe('N2 模式1：正文峰值全量互查（30%阈值/阶段门/中断）', () => {
  ...
  it('N2-15 回指阶段值不与总口径峰值互斥：「该阶段投入劳动力168人」为阶段口径，168≤216跳过（丰乐镇复测）', () => {
    expect(res('施工准备与清杂拆除阶段是后续所有作业的前置条件。该阶段投入劳动力168人，按9个自然村分组展开清杂作业。高峰期216人。')).toHaveLength(0);
  });
  it('N2-16 回指阶段值超总口径仍报出：该阶段300人 vs 高峰期216人', () => {
    const issues = res('施工准备与清杂拆除阶段是后续所有作业的前置条件。该阶段投入劳动力300人。高峰期216人。');
    expect(issues).toHaveLength(1);
  });
  it('N2-17 双回指各自解析实义阶段后隔离（4.32 丰乐镇：该阶段216人 属道路铺装工程阶段 vs 本阶段168人 属第一阶段）', () => {
    expect(res('道路铺装工程阶段处于用工峰值的第一个集中区间。该阶段同时在场人数为216人，主要作业内容为村内道路基层整平。第一阶段为施工准备与清杂拆除，占用第1日至第7日，共7天。本阶段同时在场人数168人，主要完成场地清杂。')).toHaveLength(0);
  });
  it('N2-18 双回指解析到同一实义阶段时冲突仍报：本阶段300人 vs 本阶段250人（同属第一阶段）', () => {
    const issues = res('第一阶段为施工准备与清杂拆除。本阶段同时在场人数300人。第一阶段为施工准备与清杂拆除。本阶段同时在场人数250人。');
    expect(issues).toHaveLength(1);
  });
});
