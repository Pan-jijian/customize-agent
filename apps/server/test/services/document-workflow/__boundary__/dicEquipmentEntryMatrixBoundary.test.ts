...
describe('T1 设备词 20 词全谱系（尾期阈值触发提取）', () => {
  it.each(EQUIPMENT_WORDS)('T1 设备词“%s”第168日进场 → 尾期报', (word) => {
    const issues = equipmentEntryTimingIssues(`计划工期210日历天。${word}第168日进场。`, EMPTY_FACTS);
    expect(issues).toHaveLength(1);
    expect(issues[0].message).toContain(`${word} 第168日进场`);
  });
});
...
describe('T2 第N日数字位宽边界', () => {
  ...
  it('T2-4 第060日（前导零，day=60）→ 提取并报', () => {
    expect(equipmentEntryTimingIssues('挖掘机第060日进场。计划工期75日历天。', EMPTY_FACTS)).toHaveLength(1);
  });
...
describe('T3 动作词谱系与邻接窗口边界', () => {
  it.each(ACTION_WORDS)('T3-1 动作词“%s”0字符邻接 → 尾期报', (word) => {
    expect(equipmentEntryTimingIssues(`挖掘机第168日${word}。计划工期210日历天。`, EMPTY_FACTS)).toHaveLength(1);
  });
  it('T3-2 窗口8个「，」（恰上限）→ 报', () => {
    expect(equipmentEntryTimingIssues(`挖掘机第168日${'，'.repeat(8)}进场。计划工期210日历天。`, EMPTY_FACTS)).toHaveLength(1);
  });
  it('T3-3 窗口9个「，」（超上限）→ 不报', () => {
    expect(equipmentEntryTimingIssues(`挖掘机第168日${'，'.repeat(9)}进场。计划工期210日历天。`, EMPTY_FACTS)).toEqual([]);
  });
  it('T3-4 动作词截断「投入」（缺使用）→ 不报', () => {
    expect(equipmentEntryTimingIssues('挖掘机第168日投入。计划工期210日历天。', EMPTY_FACTS)).toEqual([]);
  });
  it('T3-5 动作词截断「使用」（缺投入）→ 不报', () => {
    expect(equipmentEntryTimingIssues('挖掘机第168日使用。计划工期210日历天。', EMPTY_FACTS)).toEqual([]);
  });
  it('T3-6 「完成」非动作词 → 不报', () => {
    expect(equipmentEntryTimingIssues('挖掘机第168日完成吊装。计划工期210日历天。', EMPTY_FACTS)).toEqual([]);
  });
});
...
describe('T4 设备词前24字符窗口精确边界', () => {
  it('T4-1 3字词21填充（index=24 恰入窗）→ 报', () => {
    expect(equipmentEntryTimingIssues(`挖掘机${'支'.repeat(21)}第168日进场。计划工期210日历天。`, EMPTY_FACTS)).toHaveLength(1);
  });
  it('T4-2 3字词22填充（index=25 首字出窗）→ 不报', () => {
    expect(equipmentEntryTimingIssues(`挖掘机${'支'.repeat(22)}第168日进场。计划工期210日历天。`, EMPTY_FACTS)).toEqual([]);
  });
  it('T4-3 5字词19填充（index=24 恰入窗）→ 报', () => {
    expect(equipmentEntryTimingIssues(`塔式起重机${'支'.repeat(19)}第168日进场。计划工期210日历天。`, EMPTY_FACTS)).toHaveLength(1);
  });
  it('T4-4 5字词20填充（index=25 首字出窗）→ 不报', () => {
    expect(equipmentEntryTimingIssues(`塔式起重机${'支'.repeat(20)}第168日进场。计划工期210日历天。`, EMPTY_FACTS)).toEqual([]);
  });
  it('T4-5 词表顺序：混凝土喷射机优先于喷射机 → message 记完整词', () => {
    const issues = equipmentEntryTimingIssues('混凝土喷射机第168日进场。计划工期210日历天。', EMPTY_FACTS);
    expect(issues).toHaveLength(1);
    expect(issues[0].message).toContain('「混凝土喷射机 第168日进场」');
  });
  it('T4-6 异写「砼喷射机」→ 命中词表词「喷射机」', () => {
    const issues = equipmentEntryTimingIssues('砼喷射机第168日进场。计划工期210日历天。', EMPTY_FACTS);
    expect(issues).toHaveLength(1);
    expect(issues[0].message).toContain('「喷射机 第168日进场」');
  });
  it('T4-7 施工升降机与施工电梯（>24字符隔离防前词误绑定）→ 2 条目聚合 1 条 issue', () => {
    const issues = equipmentEntryTimingIssues(`施工升降机第168日进场。${'支'.repeat(15)}施工电梯第168日进场。计划工期210日历天。`, EMPTY_FACTS);
    expect(issues).toHaveLength(1);
    expect(issues[0].message).toContain('施工升降机');
    expect(issues[0].message).toContain('施工电梯');
  });
  it('T4-8 设备词远离25填充（整词出窗）→ 不报', () => {
    expect(equipmentEntryTimingIssues(`挖掘机${'支'.repeat(25)}第168日进场。计划工期210日历天。`, EMPTY_FACTS)).toEqual([]);
  });
});

// ── T5. 尾期阈值 total*0.8 精确边界 ──

describe('T5 尾期阈值 total*0.8 精确边界', () => {
  it('T5-1 total=210 day=168 恰阈值 → 报', () => {
    expect(equipmentEntryTimingIssues('挖掘机第168日进场。计划工期210日历天。', EMPTY_FACTS)).toHaveLength(1);
  });
  it('T5-2 total=210 day=167 → 不报', () => {
    expect(equipmentEntryTimingIssues('挖掘机第167日进场。计划工期210日历天。', EMPTY_FACTS)).toEqual([]);
  });
  it('T5-3 total=100 day=80 恰阈值 → 报', () => {
    expect(equipmentEntryTimingIssues('挖掘机第80日进场。计划工期100日历天。', EMPTY_FACTS)).toHaveLength(1);
  });
  ...
  it('T5-6 total=101 day=81（≥80.8）→ 报', () => {
    expect(equipmentEntryTimingIssues('挖掘机第81日进场。计划工期101日历天。', EMPTY_FACTS)).toHaveLength(1);
  });
  it('T5-7 total=30（有效域下限）day=24 恰阈值 → 报', () => {
    expect(equipmentEntryTimingIssues('挖掘机第24日进场。计划工期30日历天。', EMPTY_FACTS)).toHaveLength(1);
  });
  ...
  it('T5-10 total=1200 day=960 恰阈值 → 报', () => {
    expect(equipmentEntryTimingIssues('挖掘机第960日进场。计划工期1200日历天。', EMPTY_FACTS)).toHaveLength(1);
  });
  ...
  it('T5-12 total=3001（>3000 无效）→ 不报', () => {
    expect(equipmentEntryTimingIssues('挖掘机第168日进场。计划工期3001日历天。', EMPTY_FACTS)).toEqual([]);
  });
  it('T5-13 total=29（<30 无效，total=undefined）→ 不报', () => {
    expect(equipmentEntryTimingIssues('挖掘机第168日进场。计划工期29日历天。', EMPTY_FACTS)).toEqual([]);
  });
});
...
describe('T7 组合矩阵与聚合', () => {
  it('T7-1 尾期+倒挂同文同报 → 2 条', () => {
    const md = '计划工期210日历天。第60日完成基坑支护及土方外运。挖掘机第168日进场。';
    expect(equipmentEntryTimingIssues(md, EMPTY_FACTS)).toHaveLength(2);
  });
  it('T7-2 2 台设备尾期（>24字符隔离防前词误绑定）→ 聚合 1 条 issue 含双条目', () => {
    const md = `计划工期210日历天。挖掘机第168日进场。${'支'.repeat(15)}空压机第170日进场。`;
    const issues = equipmentEntryTimingIssues(md, EMPTY_FACTS);
    expect(issues).toHaveLength(1);
    expect(issues[0].message).toContain('挖掘机 第168日进场');
    expect(issues[0].message).toContain('空压机 第170日进场');
  });
  it('T7-3 4 台设备尾期 → message 展示截断 slice(0,3)，第 4 台不入 message', () => {
    const md = `计划工期210日历天。挖掘机第168日进场。${'支'.repeat(15)}装载机第168日进场。${'支'.repeat(15)}压路机第168日进场。${'支'.repeat(15)}平地机第168日进场。`;
    const issues = equipmentEntryTimingIssues(md, EMPTY_FACTS);
    expect(issues).toHaveLength(1);
  ...
  it('T7-5 同 equipment:raw 重复段 → 去重为 1 条目', () => {
    const md = 'ABCDEFGHIJKLMNOPQ挖掘机第168日进场ABCDEFGHIJKLMNOPQ挖掘机第168日进场。计划工期210日历天。';
    const issues = equipmentEntryTimingIssues(md, EMPTY_FACTS);
    expect(issues).toHaveLength(1);
    expect(issues[0].message.match(/第168日进场/gu)).toHaveLength(1);
  });
  it('T7-6 空 markdown → 不报', () => {
    expect(equipmentEntryTimingIssues('', EMPTY_FACTS)).toEqual([]);
  });
  it('T7-7 无设备词句 → 不报', () => {
    expect(equipmentEntryTimingIssues('计划工期210日历天。第168日完成主体结构封顶。', EMPTY_FACTS)).toEqual([]);
  });
...
describe('T8 动作词窗口排除字符谱系', () => {
  it('T8-1 窗口首字符「|」→ 不匹配不报', () => {
    expect(equipmentEntryTimingIssues('挖掘机第168日|进场。计划工期210日历天。', EMPTY_FACTS)).toEqual([]);
  });
  it('T8-2 窗口首字符「。」→ 不报', () => {
    expect(equipmentEntryTimingIssues('挖掘机第168日。进场。计划工期210日历天。', EMPTY_FACTS)).toEqual([]);
  });
  ...
  it('T8-5 窗口首字符换行 → 不报', () => {
    expect(equipmentEntryTimingIssues('挖掘机第168日\n进场。计划工期210日历天。', EMPTY_FACTS)).toEqual([]);
  });
});
...
describe('T10 总工期锚点谱系与窗口边界', () => {
  ...
  it.each(ANCHORS)('T10-1 锚点“%s”210日历天 → 尾期报', (anchor) => {
    expect(equipmentEntryTimingIssues(`挖掘机第168日进场。${anchor}210日历天。`, EMPTY_FACTS)).toHaveLength(1);
  });
  it('T10-2 「个」可选：「计划工期210个日历天」→ 报', () => {
    expect(equipmentEntryTimingIssues('挖掘机第168日进场。计划工期210个日历天。', EMPTY_FACTS)).toHaveLength(1);
  });
  it('T10-3 锚点前「约」1字符入窗口 → 报', () => {
    expect(equipmentEntryTimingIssues('挖掘机第168日进场。计划工期约210日历天。', EMPTY_FACTS)).toHaveLength(1);
  });
  it('T10-4 锚点后 12 字符（恰上限）→ 报', () => {
    expect(equipmentEntryTimingIssues(`挖掘机第168日进场。计划工期${'X'.repeat(12)}210日历天。`, EMPTY_FACTS)).toHaveLength(1);
  });
  it('T10-5 锚点后 13 字符（超上限，数字被窗口吞）→ 不报', () => {
    expect(equipmentEntryTimingIssues(`挖掘机第168日进场。计划工期${'X'.repeat(13)}210日历天。`, EMPTY_FACTS)).toEqual([]);
  });
  it('T10-6 锚点后直接句号截断 → total undefined 不报', () => {
    expect(equipmentEntryTimingIssues('挖掘机第168日进场。计划工期。210日历天。', EMPTY_FACTS)).toEqual([]);
  });
  it('T10-7 中文数字「二百一十」（非 \d）→ total undefined 不报', () => {
    expect(equipmentEntryTimingIssues('挖掘机第168日进场。计划工期二百一十日历天。', EMPTY_FACTS)).toEqual([]);
  });
  it('T10-8 锚点后「—」特殊字符入窗口 → 报', () => {
    expect(equipmentEntryTimingIssues('挖掘机第168日进场。计划工期—210日历天。', EMPTY_FACTS)).toHaveLength(1);
  });
  it('T10-9 「计划总工期」含「总工期」锚点子串 → 报', () => {
    expect(equipmentEntryTimingIssues('挖掘机第168日进场。计划总工期210日历天。', EMPTY_FACTS)).toHaveLength(1);
  });
});
...
describe('T10 factsModel.schedule 兜底与正文优先', () => {
  it('T10-10 正文无锚点，facts value 含锚点词 → 兜底 total=210 报', () => {
    ...
    expect(equipmentEntryTimingIssues('挖掘机第168日进场。', facts)).toHaveLength(1);
  });
  ...
  it('T10-12 facts value 无锚点词 → 兜底无效不报', () => {
    ...
    expect(equipmentEntryTimingIssues('挖掘机第168日进场。', facts)).toEqual([]);
  });
  it('T10-13 正文300优先于 facts210 → day=240 报', () => {
    const facts = factsOf({ schedule: [factOf({ value: '计划工期210日历天' })] });
    expect(equipmentEntryTimingIssues('挖掘机第240日进场。合同工期300日历天。', facts)).toHaveLength(1);
  });
