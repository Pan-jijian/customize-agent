...
- 4.44.0 门禁链源头根治（丰乐镇 4.43 实测 45 项阻断全量定因 + 全链落码）：
  ...
  - server 数值/结构/语义类缺陷根治（A1-A8 + B1 + 结构双修）：
    ...
    - D 法规文号残缺链尾收口（丰乐镇实机两轮同形态残留「（国务院令第279订）」）：stage5 全文数值链（含文号修复）之后的 LLM patch 轮（数值/要求定向修复、缺节补写）重写句段可再度引入自吞噬文号残缺，链尾此前无确定性收口点——postReviewSurface 链尾（terminology-strip 后、toc-consistency 前）接入 fixRegulationNumberTypos 确定性收口（仅「第N订」紧邻右括号形态、幂等零误伤），FINALIZE_REPAIR_ROUNDS 登记 14 轮
