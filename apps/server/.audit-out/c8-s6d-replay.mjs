...
//#endregion
//#region apps/server/src/services/document-workflow/finalize/repairRounds/templatingTailReplay.ts
function replayTemplatingOnMarkdown(markdown) {
...
var init_templatingTailReplay = __esmMin((() => {
	init_documentIntegrityChecks();
	init_constructionOrgAudit();
}));
//#endregion
//#region .dbg/c8-s6d-replay.ts
/**
* C8 S6d replay（正式实现重放对照，templating-tail-replay 落码后实机复验）：
* 导入正式轮实现 replayTemplatingOnMarkdown 对 s28m'/r28m' 终稿重放 + 合成样本触发能力复验，
* 与 .dbg/c8-s6-probe.ts（独立蓝本探针，输出已落盘 c8-s6-probe-output.txt）逐项对照——
* 证明正式实现与探针预期等价：
* ① 两样本终稿：前缀 targets=0/删除 0/段落 0/长度不变 + 幂等 0/0（零残留、零误伤）；
* ② 合成样本：targets=3、prefixRemoved=2（含数字岗位句被零信息硬闸拒删）、paragraphRemoved=1
*   （章内重复段；跨章复用段章内作用域保留 2）。
* 运行：node node_modules/.pnpm/rolldown@1.0.3/node_modules/rolldown/bin/cli.mjs -c .dbg/rolldown-c8-s6d-replay.config.mjs
*   && node apps/server/.audit-out/c8-s6d-replay.mjs
*/
var require_c8_s6d_replay = /* @__PURE__ */ __commonJSMin((() => {
	init_templatingTailReplay();
	const DOCS = {
	...
	for (const [name, file] of Object.entries(DOCS)) {
		...
		console.log(`\n===== ${name}（正式实现重放）=====`);
		console.log(`前缀句 targets=${result.targetCount}；删除 ${result.prefixRemoved} 处（蓝本对照：0/0）`);
		for (const sentence of result.prefixSentences.slice(0, 6)) console.log(`   前缀句删除：「${sentence.slice(0, 60)}」`);
		console.log(`段落去重删除 ${result.paragraphRemoved} 处（蓝本对照：0）`);
		console.log(`长度：${markdown.length} → ${result.markdown.length}（变化 ${markdown.length - result.markdown.length}，蓝本对照：0）`);
		const second = replayTemplatingOnMarkdown(result.markdown);
		console.log(`幂等核对：二次重放 前缀 ${second.prefixRemoved} 处 / 段落 ${second.paragraphRemoved} 处（期望 0/0）`);
	}
	{
		console.log("\n===== 合成样本（正式实现触发复验）=====");
		const result = replayTemplatingOnMarkdown(SYNTHETIC);
		const after = result.markdown.split("\n");
		console.log(`前缀句 targets=${result.targetCount}（蓝本对照：3）；删除 ${result.prefixRemoved} 处（蓝本对照：2）`);
		for (const sentence of result.prefixSentences) console.log(`   删：「${sentence}」`);
		console.log(`段落去重 ${result.paragraphRemoved} 处（蓝本对照：1）`);
...
//#endregion
export default require_c8_s6d_replay();
export {};
