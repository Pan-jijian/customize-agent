...
function basisRegulationSectionRanges(markdown) {
	const lines = markdown.split(/\r?\n/u);
	const rangeEndFrom = (startIndex, level) => {
	...
	const ranges = [];
	const pushRange = (start, level, title) => {
		if (ranges.some((range) => range.start === start)) return;
		ranges.push({
			start,
			end: rangeEndFrom(start, level),
			level,
			title
		});
	};
...
* 排除区段复用）。 */
function extractBasisRegulationSection(markdown) {
	const lines = markdown.split(/\r?\n/u);
	return basisRegulationSectionRanges(markdown).map((range) => [range.title, ...lines.slice(range.start + 1, range.end)].join("\n")).join("\n");
}
...
function normalizeRegulationCode(raw) {
	return raw.replace(/[\s\u200b\u200c\u200d\ufeff]/gu, "").replace(/[—–－]/gu, "-").toUpperCase();
}
/** 书名归一：去书名号、括号注释（如「（2011年版）」）、空白与零宽字符（保留汉字本体供包含匹配） */
function normalizeRegulationName(raw) {
	return raw.replace(/《|》/gu, "").replace(/[（(][^）)]{0,40}[）)]/gu, "").replace(/[\s\u200b\u200c\u200d\ufeff]/gu, "");
}
/** 技术标准类书名判定（归一后以 规范/标准/规程 结尾） */
function isTechnicalStandardName(name) {
...
function sameRegulationCodeFamily(left, right) {
...
function regulationNameMatches(left, right) {
...
function pairLineEntries(text) {
	...
	for (const line of text.split(/\r?\n/u)) {
		...
		const pairedName = /* @__PURE__ */ new Map();
		const pairedNameIndexes = /* @__PURE__ */ new Set();
		tokens.forEach((token, tokenIndex) => {
			if (token.type !== "code") return;
			const findName = (direction) => {
				const candidates = direction === "left" ? tokens.slice(0, tokenIndex).map((_, offset) => tokenIndex - 1 - offset) : tokens.slice(tokenIndex + 1).map((_, offset) => tokenIndex + 1 + offset);
		...
		tokens.forEach((token, tokenIndex) => {
			...
			if (token.type === "code") {
				...
				} else entries.push({ code: token.value });
...
function evidenceContext(text, token) {
...
function gapSameFamily(left, right) {
...
function auditBasisRegulationsCross(markdown) {
	const ranges = basisRegulationSectionRanges(markdown);
	const sectionText = extractBasisRegulationSection(markdown);
	const lines = markdown.split(/\r?\n/u);
	const excluded = /* @__PURE__ */ new Set();
	for (const range of ranges) for (let index = range.start; index < range.end; index += 1) excluded.add(index);
	...
	const codeUsedInBody = (code) => bodyCodes.some((bodyCode) => sameRegulationCodeFamily(code, bodyCode));
	const nameUsedInBody = (name) => bodyNames.some((bodyName) => regulationNameMatches(name, bodyName));
	const codeDeclared = (code) => declaredCodes.some((declared) => sameRegulationCodeFamily(declared, code));
	const nameDeclared = (name) => declaredNames.some((declared) => regulationNameMatches(declared, name));
	const declaredRaw = [];
	for (const entry of declaredEntries) {
		...
		if (entry.code && !codeHit && entry.name && isTechnicalStandardName(entry.name)) declaredRaw.push({
			kind: "pair",
			value: entry.name,
			code: entry.code,
			name: entry.name,
			counterpart: entry.code
		});
		else if (entry.code && !codeHit) declaredRaw.push({
			kind: "code",
			value: entry.code,
			code: entry.code,
			counterpart: entry.name
		});
		else if (entry.name && isTechnicalStandardName(entry.name) && !nameHit) declaredRaw.push({
			kind: "name",
			value: entry.name,
			name: entry.name,
			counterpart: entry.code
	...
	for (const entry of bodyEntries) {
		...
		if (entry.code && !codeHit && entry.name && isTechnicalStandardName(entry.name)) usedRaw.push({
			kind: "pair",
			value: entry.name,
			code: entry.code,
			name: entry.name,
			context: evidenceContext(bodyText, entry.code)
		});
		else if (entry.code && !codeHit) usedRaw.push({
			kind: "code",
			value: entry.code,
			code: entry.code,
			context: evidenceContext(bodyText, entry.code)
		});
		else if (entry.name && isTechnicalStandardName(entry.name) && !nameHit) usedRaw.push({
			kind: "name",
			value: entry.name,
			name: entry.name,
			context: evidenceContext(bodyText, `《${entry.name}》`)
		});
	}
	return {
...
function basisRegulationsCrossIssues(markdown) {
	...
	for (const gap of usedGaps.slice(0, MAX_CROSS_ISSUES_PER_DIRECTION)) issues.push({
		level: "error",
		severity: "blocker",
		category: "fact_consistency",
		owner: "llm",
	...
	for (const gap of declaredGaps.slice(0, MAX_CROSS_ISSUES_PER_DIRECTION)) issues.push({
		level: "warning",
		category: "fact_consistency",
		owner: "llm",
		repairability: "llm_repairable",
...
var REGULATION_CODE_RE, REGULATION_BOOK_RE, TECHNICAL_STANDARD_TAIL_RE, NAME_CONTAINS_MIN_LENGTH, PAIR_CONNECTOR_RE, MAX_CROSS_ISSUES_PER_DIRECTION;
var init_basisRegulationsCross = __esmMin((() => {
	init_qualityValidation();
	REGULATION_CODE_RE = /(?:GB|JGJ|CJJ|JTG|SL|DB|DL|YS|HG|DBJ)[\s/T]*\d{1,2}[\s/-]?(?:T[\s/]*)?\d{2,4}(?:[\s/-]\d{1,4})?/gu;
	REGULATION_BOOK_RE = /《[^《》\r\n]{2,60}?》/gu;
	TECHNICAL_STANDARD_TAIL_RE = /(?:规范|标准|规程)$/u;
	NAME_CONTAINS_MIN_LENGTH = 4;
	PAIR_CONNECTOR_RE = /^[\s（）()【】\[\]、,，;；:：·.及和与]*$/u;
	MAX_CROSS_ISSUES_PER_DIRECTION = 12;
}));
//#endregion
...
//#endregion
export { ambiguousEitherOrIssues as C, SELF_UNDERMINING_PROCEDURAL_EXEMPT_RE as S, init_basisRegulationsCross as _, getQueuedDocumentPosition as a, init_qualityValidation as b, listGeneratedDocuments as c, upsertGeneratedDocumentAsset as d, buildGenerationBudget as f, basisRegulationsCrossIssues as g, auditBasisRegulationsCross as h, getGeneratedDocument as i, saveGeneratedDocument as l, previewGenerationBudgetForTemplate as m, init_fixers as n, init_generatedDocumentService as o, init_generationBudget as p, generatedRoot as r, listGeneratedAssets as s, fixSelfUnderminingCandidates as t, upsertGeneratedAssets as u, conceptConflictGroups as v, init_detectors as w, scanTablePlaceholderCells as x, init_parameterConceptConflicts as y };
