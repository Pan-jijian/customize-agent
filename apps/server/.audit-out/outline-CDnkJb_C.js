...
//#endregion
//#region apps/server/src/services/document-validation/engineeringDocumentConfigService.ts
function configPath() {
	const dir = path$1.join(os$1.homedir(), ".customize-agent");
	fs$1.mkdirSync(dir, { recursive: true });
	return path$1.join(dir, "engineering-document-config.json");
}
...
function readEngineeringDocumentConfig() {
	try {
		const file = configPath();
		if (!fs$1.existsSync(file)) return DEFAULT_CONFIG;
		return normalizeConfig(JSON.parse(fs$1.readFileSync(file, "utf-8")));
	} catch {
...
var MARKDOWN_TABLE_ROW_RE, MARKDOWN_TABLE_DIVIDER_RE, LINE_SPLIT_RE, FORMAL_PLACEHOLDER_PATTERNS, TABLE_PLACEHOLDER_CELL_FORMS_RE, TABLE_PLACEHOLDER_APPROX_RE;
var init_qualityValidationConstants = __esmMin((() => {
	MARKDOWN_TABLE_ROW_RE = /\|.+\|/u;
	MARKDOWN_TABLE_DIVIDER_RE = /^\s*\|?\s*:?-{3,}:?\s*(?:\|\s*:?-{3,}:?\s*)+\|?\s*$/u;
	LINE_SPLIT_RE = /\r?\n/u;
	...
	TABLE_PLACEHOLDER_CELL_FORMS_RE = /^(?:—+|-+|-|\/|N\/A|n\/a|待定|待补充|待确认|待查|待补|若干|暂无|无数据|无)$/u;
	TABLE_PLACEHOLDER_APPROX_RE = /^约\d/u;
}));
//#endregion
...
function getWorkspaceRoot() {
	return process.env.INIT_CWD && !isInternalResidualProject(process.env.INIT_CWD) ? process.env.INIT_CWD : path.resolve(process.cwd(), "../..");
}
...
function isInternalResidualProject(projectRoot) {
	...
	return normalized === homeConfig || normalized.includes(`${path.sep}.customize-agent${path.sep}`) || normalized.endsWith(`${path.sep}apps${path.sep}server`) || normalized.endsWith(`${path.sep}apps${path.sep}cli`);
}
function getKnownProjectRoots() {
	...
	try {
		return db.prepare("SELECT project_root FROM project_registry ORDER BY last_opened_at DESC").all().map((r) => path.resolve(r.project_root)).filter((root) => !isInternalResidualProject(root));
	} finally {
		db.close();
	}
}
...
var init_workflowRules = __esmMin((() => {
	DEFAULT_WORKFLOW_RULES = {
		...
		writingSpec: {
			...
			writeRules: {
				majorContent: "【项目主要施工内容专项结构】只能根据绑定材料中的当前项目事实识别施工对象和工作包；不得套用固定行业模板，不得复述完整工程概况，不得写“以图纸清单为准”式空话；不得使用 Markdown 表格。必须按专业工程/分部分项工程逐项展开，每个专业工程单独一个“#### 工作包名称”三级小节（不得把所有专业工程合并成一个小节），内容覆盖三方面要素：①作业对象与工程量（什么部位、什么规模）、②工序安排（先后顺序清晰）、③施工方法（怎么干、用什么参数验收）。呈现形式要求：三方面要素必须融入连贯散文叙述，禁止以“施工概况/施工流程/施工方法”等结构标签充当小节标题或段落开头引导（不得出现“施工概况：……”式标签行），标签词只是后台结构概念、不得入题入文。工作包小节必须与上下文“主要施工工作包”列表一一对应，每个工作包只允许展开一次；严禁把同一个工作包以“X工程”“X工作包”两种口径重复写成两个小节，也不得新增图谱之外的工作包小节。作业对象与工程量要素必须写该工作包对应的本项目作业对象、部位、规模/工程量、材料设备或系统边界，写成连贯叙述，避免“xxx｜工程量”式清单原文罗列；工程量数值必须与工程量清单汇总值一致（只写项目总量，禁止写分部小计、单村分表量或估算值）。工序要素必须有明确的工序顺序表达（相邻小节不得同句式开头，同一句式全文不得反复使用，禁止以固定句模复读），每个工作包至少 1 处不少于 4 个环节的工序顺序表达，不得只把工序顺序局限在一处标签段；施工方法要素写成连贯叙述，落到具体工具机具、测量/检测方法、工艺参数、材料规格、穿插关系、质量验收、复试检测和资料闭环，每个工作包施工方法宜落位至少 3 个具体工艺参数（厚度、间距、偏差、含水率、饱满度、坡度、压实度等），参数来自绑定材料或行业通用规范值，禁止“按规范施工”“结合实际执行”式空话，严禁把工程量清单条目原样罗列成“xxx：2台；xxx：1台；”式参数堆砌。严禁出现工程量清单计价表内部口径词：分部小计、本页小计、合计、小计、按实、暂估、综合单价、措施项目费、规费、税金（这些是清单计价表专用标记，正式正文一律不得出现；“措施项目”作为专业工程类别名称可用，但不得与“清单/计价/费”连用）。施工方法写法要求：按“怎么做/用什么/参数多少/怎么验收”四要素落位，逐项写出具体做法；句式自然多样，严禁套用固定句式模板或段落间同句式复读。至少形成 5 个施工工作包，工作包必须来自绑定材料证据。本小节正文总量不少于 2200 字（每个工作包小节按 300 字以上展开，写足三要素、工序细节与验收要求，防止内容深度不足触发导出阻断）。",
...
function mergeTableLineBreaks(markdown) {
	const lines = markdown.split(NEWLINE_SPLIT_RE);
	const output = [];
	for (let index = 0; index < lines.length; index += 1) {
		const line = lines[index] || "";
		const trimmed = line.trim();
		if (!(Boolean(trimmed) && !/^\|/u.test(trimmed) && (trimmed.match(/\|/gu) || []).length >= 2 && !/^#{1,6}\s/u.test(trimmed) && !isMarkdownTableDivider(trimmed) && output.length > 0 && isMarkdownTableRow(output[output.length - 1] || ""))) {
			output.push(line);
			continue;
		}
		...
		if (isMarkdownTableDivider(output[output.length - 1] || "")) {
			output.push(`| ${parts.join(" | ")} |`);
			continue;
		}
		const mergedCell = `${(output[output.length - 1] || "").replace(/\s*\|\s*$/u, "")}${parts[0]}`;
...
var INLINE_LIST_MARKER_RE, CR_CHAR, LF_CHAR, NEWLINE_SPLIT_RE, SOURCE_ENUMERATION_PHRASE_RE, MARKDOWN_TABLE_FORMAT_RULES, TENDER_BID_WRITING_RULES, FORMAL_WRITING_RULES, SECTION_GENERATION_SAFETY_RULES, DOCUMENT_L0_COMMON_PREFIX;
var init_markdownComposer = __esmMin((() => {
	...
	NEWLINE_SPLIT_RE = new RegExp(`${CR_CHAR}?${LF_CHAR}`, "u");
...
//#endregion
//#region apps/server/src/services/document-workflow/templateStore.ts
...
function configuredChapterTitleFilters(template) {
...
function violatesConfiguredChapterTitleForbiddenFilter(title, template) {
...
//#endregion
//#region apps/server/src/services/document-workflow/outline.ts
...
function normalizeGeneratedChapterTitle(title) {
	return displayChapterTitle(title.replace(/\s+/gu, " ").trim()).replace(/^[，,、；;：:。.!！?？\-—\s]+/u, "").trim();
}
function isPollutedChapterTitle(title) {
	return /见(?:公告|文件|资料|附件)|按(?:资料|文件|相关要求)|质量标准[:：]|范围[:：].*依据/u.test(title);
}
function uniqueTemplateChapters(chapters, options) {
...
function effectiveTemplateChapters(template, spec, options) {
	if (!spec || options?.preserveExplicitOutline) return uniqueTemplateChapters([...template.chapters], {
		...options,
		template
	});
...
//#endregion
export { BID_DISCIPLINE_PHRASES as A, init_semanticSimilarity as B, init_tuningProfile as C, init_documentRoleService as D, init_projectMaterialService as E, explicitLengthTargets as F, MARKDOWN_TABLE_ROW_RE as G, FORMAL_PLACEHOLDER_PATTERNS as H, init_budget as I, init_qualityValidationConstants as J, TABLE_PLACEHOLDER_APPROX_RE as K, SEMANTIC_COVERAGE_THRESHOLD as L, stableHash as M, stringifyFactValue as N, getProjectRoot as O, documentTextLength as P, buildSemanticSimilarity as R, init_llmClient as S, init_configService as T, LINE_SPLIT_RE as U, init_constants$1 as V, MARKDOWN_TABLE_DIVIDER_RE as W, __esmMin as X, __commonJSMin as Y, init_writingSpec as _, normalizePlannedSectionTitle as a, init_semanticGate as b, init_projectMaterialProfile as c, init_factMatching as d, init_projectGraph as f, init_constructionOrgTablePlan as g, mergeTableLineBreaks as h, normalizeGeneratedChapterTitle as i, init_utils as j, init_kbService as k, init_agentWorkflow as l, init_markdownComposer as m, effectiveTemplateChapters as n, uniqueTemplateChapters as o, docSystemPrefix as p, TABLE_PLACEHOLDER_CELL_FORMS_RE as q, init_outline as r, init_templateStore as s, displayChapterTitle as t, init_evidence as u, init_workflowRules as v, tuningProfile as w, concurrencyForDocumentScale as x, buildSemanticGate as y, getLocalSemanticProvider as z };
