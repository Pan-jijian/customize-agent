import { A as init_constants, C as init_tuningProfile, D as init_documentRoleService, E as init_projectMaterialService, I as init_budget, M as init_utils, N as stringifyFactValue, O as getProjectRoot, P as documentTextLength, R as __esmMin, S as init_llmClient, T as init_configService, _ as init_outline$1, a as init_factMatching, c as init_markdownComposer, d as init_workflowRules, f as buildSemanticGate, i as init_evidence, j as BID_DISCIPLINE_PHRASES, k as init_kbService, l as init_constructionOrgTablePlan, m as init_semanticSimilarity, n as init_projectMaterialProfile, o as init_projectGraph, p as init_semanticGate, r as init_agentWorkflow, s as docSystemPrefix, t as init_templateStore, u as init_writingSpec, w as tuningProfile, x as concurrencyForDocumentScale } from "./templateStore-DlqMBWhl.js";
import { a as init_promptRuleExtraction, o as init_sectionNamingGovernance, s as init_sectionFingerprint } from "./promptRuleExtraction-CrEJqE37.js";
...
var HAZARD_CATEGORY_TERMS, NL, DESIGN_PARAM_WORD_RE;
...
async function buildSpecActionGate(embedDocuments) {
	return buildSemanticGate({
...
async function collectLayerNumbers(text, actionGate) {
	const claims = [];
	const matches = [...text.matchAll(SPEC_LAYER_RE)];
	const usedRanges = [];
	const isUsed = (start, end) => usedRanges.some(([s, e]) => start < e && end > s);
	const uncertainGaps = [];
	const claim = (window, offset, re, direction, layer, kind) => {
		...
		for (const m of ordered) {
			...
			if (kind === "thickness") {
				...
				if (!Number.isFinite(rawValue) || rawValue <= 0 || rawValue >= 1e3) continue;
			}
			if (actionGate && gap.trim()) uncertainGaps.push({
				key: `${layer}|${absStart}|${absEnd}`,
				gap
			});
	...
	for (let i = 0; i < matches.length; i += 1) {
		...
		const afterEnd = nextLayerStart === void 0 ? Math.min(text.length, layerEnd + 90) : nextLayerStart;
...
async function layeredSpecEntries(text, actionGate) {
	const entries = [];
	const seen = /* @__PURE__ */ new Set();
	for (const claim of await collectLayerNumbers(text, actionGate)) {
		const ratio = claim.kind === "ratio" ? claim.raw.match(/(\d+:\d+(?:\.\d+)?)/u)?.[1] : void 0;
		const thicknessMatch = claim.kind === "thickness" ? claim.raw.match(/(\d+(?:\.\d+)?)/u)?.[1] : void 0;
		const thickness = thicknessMatch ? Number(thicknessMatch) : void 0;
...
async function deterministicFixTargets(factsModel, scopeConflicts, embedDocuments) {
	...
	const sourceByLayer = /* @__PURE__ */ new Map();
	for (const entry of await layeredSpecEntries(sourceText, actionGate)) {
		...
		if (Number.isFinite(entry.thickness) && !list.thicknesses.includes(entry.thickness)) list.thicknesses.push(entry.thickness);
		sourceByLayer.set(entry.layer, list);
	}
	const specTargets = /* @__PURE__ */ new Map();
	for (const [layer, list] of sourceByLayer) {
		if (list.ratios.length !== 1 && list.thicknesses.length !== 1) continue;
		specTargets.set(layer, {
			ratio: list.ratios.length === 1 ? list.ratios[0] : void 0,
			thickness: list.thicknesses.length === 1 ? list.thicknesses[0] : void 0
		});
	}
	...
	return {
		specTargets,
		scaleTarget: expectedScale ? areaResolution ? numericEntryFromResolution(areaResolution) : await resolveScaleExpectation(expectedScale, scaleGapGate) : void 0,
		costTarget: expectedCost ? costResolution ? numericEntryFromResolution(costResolution) : (await scopedNumericEntries(expectedCost, COST_SCOPE_RE, COST_UNIT_RE, COST_GAP_WORDS_RE, void 0, costGapGate)).entries[0] : void 0,
		scaleGapGate,
		costGapGate,
		actionGate
	};
...
async function fixChapterDeterministic(text, targets) {
	const replacements = [];
	for (const claim of await collectLayerNumbers(text, targets.actionGate)) {
		...
		if (claim.kind === "ratio" && target.ratio) {
			...
			if (ratio && ratio !== target.ratio) replacements.push({
				start: claim.span[0],
				end: claim.span[1],
				replacement: claim.raw.replace(/(\d+:\d+(?:\.\d+)?)/u, target.ratio),
				detail: `${claim.layer}配比 ${ratio}→${target.ratio}`
			});
		}
		if (claim.kind === "thickness" && Number.isFinite(target.thickness)) {
			const thickness = Number(claim.raw.match(/(\d+(?:\.\d+)?)/u)?.[1]);
			if (Number.isFinite(thickness) && thickness !== target.thickness) replacements.push({
				start: claim.span[0],
				end: claim.span[1],
				replacement: claim.raw.replace(/(\d+(?:\.\d+)?)/u, String(target.thickness)),
				detail: `${claim.layer}厚度 ${thickness}mm→${target.thickness}mm`
			});
		}
	}
	const collectScopeSpans = async (target, scopeRe, unitRe, kindLabel, gapWords, prefixWords, gapGate) => {
		if (!target) return;
		const pattern = new RegExp(`(?:${scopeRe.source})(?:[^\\n。；;，,]{0,14}?)(\\d{2,}(?:[.,]\\d+)?\\s*万?)\\s*(${unitRe.source})`, "giu");
		const matches = [...text.matchAll(pattern)];
		const uncertain = gapGate ? matches.filter((match) => {
		...
		for (const match of matches) {
			...
			const entry = {
				value: match[1].replace(/[,，]/gu, "").replace(/\s+/gu, ""),
				unit: match[2]
			};
			...
			replacements.push({
				...
				detail: `${kindLabel} ${entry.value}${entry.unit}→${target.value}${entry.unit}`
			});
		}
	};
	...
	await collectScopeSpans(targets.costTarget, COST_SCOPE_RE, COST_UNIT_RE, "估算价", COST_GAP_WORDS_RE, void 0, targets.costGapGate);
	...
	return {
		...
		details: replacements.map((item) => item.detail)
	};
}
/** 确定性定点修复兜底：LLM 定向修复未消除的数值口径冲突，按检测同源归属规则直接替换为资料口径（原地修改 chapters 内容） */
async function applyDeterministicConsistencyFixes(chapters, factsModel, scopeConflicts, embedDocuments) {
	const targets = await deterministicFixTargets(factsModel, scopeConflicts, embedDocuments);
	if (targets.specTargets.size === 0 && !targets.scaleTarget && !targets.costTarget) return {
		fixedCount: 0,
		details: []
	};
	let fixedCount = 0;
...
var SCALE_GAP_WORDS_RE, SCALE_PREFIX_WORDS_RE, COST_GAP_WORDS_RE, SCALE_GAP_SEMANTIC_PROTOTYPES, SCALE_GAP_LEGAL_PROTOTYPES, COST_GAP_SEMANTIC_PROTOTYPES, COST_GAP_LEGAL_PROTOTYPES, SCALE_SCOPE_RE, SCALE_UNIT_RE, COST_SCOPE_RE, COST_UNIT_RE, SPEC_LAYER_RE, SPEC_ACTION_RE, SPEC_ACTION_SEMANTIC_PROTOTYPES, SPEC_ACTION_LEGAL_PROTOTYPES;
var init_qualityValidation = __esmMin((() => {
	...
	SPEC_LAYER_RE = /找平层|抹灰层|防水层|保温层|结合层|垫层|面层|粘结层|隔热层|隔离层/gu;
	SPEC_ACTION_RE = /铺设|施工|浇筑|粘贴|铺贴|涂抹|完成|进行|待|设置|铺装|挂网|喷涂|灌注/u;
	SPEC_ACTION_SEMANTIC_PROTOTYPES = ["施工过程与工序动作的先后描述", "铺设浇筑完成后再进行的工序"];
	SPEC_ACTION_LEGAL_PROTOTYPES = ["结构层的厚度与配比参数", "找平层厚20毫米的规格"];
}));
//#endregion
...
function locateDecisionOptionAnchor(category, text) {
...
function fixAmbiguousEitherOrCandidates(markdown, options) {
	let result = markdown;
	let fixedCount = 0;
	const details = [];
	...
	if (options?.decisionLock?.length) {
		...
		for (const match of result.matchAll(/([一-龥]{2,12})或([一-龥]{2,12})/gu)) {
			...
			const target = category.options.find((option) => lockEntry.values.includes(option.value) && (option.aliases.test(match[1]) || option.aliases.test(match[2])))?.value ?? lockEntry.values[0];
			decisionHits.push({
...
var AMBIGUOUS_DECISION_FIXES;
var init_fixers = __esmMin((() => {
	init_constants();
	init_integratedBlueprint();
	init_detectors();
	AMBIGUOUS_DECISION_FIXES = [
		{
			from: /钢板桩或型钢支撑(?:支护)?/gu,
			sides: [{
				text: "钢板桩支护",
		...
		},
		{
			...
			detail: "放坡钢板桩两可归一为组合支护"
		},
		{
			from: /打桩机打入或混凝土基础固定/gu,
			sides: [{
				text: "打桩机打入",
				keyword: /打入|打桩/
			}, {
				text: "混凝土基础固定",
				keyword: /混凝土基础|现浇|基础固定/
			}],
			detail: "护栏立柱安装两可归一为打桩机打入"
		},
		{
			from: /放坡或加设挡板支护/gu,
			sides: [{
...
//#endregion
export { init_qualityValidation as _, getGeneratedDocument as a, listGeneratedAssets as c, upsertGeneratedAssets as d, upsertGeneratedDocumentAsset as f, applyDeterministicConsistencyFixes as g, previewGenerationBudgetForTemplate as h, generatedRoot as i, listGeneratedDocuments as l, init_generationBudget as m, fixSelfUnderminingCandidates as n, getQueuedDocumentPosition as o, buildGenerationBudget as p, init_fixers as r, init_generatedDocumentService as s, fixAmbiguousEitherOrCandidates as t, saveGeneratedDocument as u };
