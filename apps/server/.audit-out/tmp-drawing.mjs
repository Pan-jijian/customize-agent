...
//#endregion
//#region .dbg/tmp-c3-probe-drawing.ts
/**
* C3-6 临时探查：离线重建 drawingFactLock（理解阶段 evidence 确定性通道复现探针）。
* 目标：复现运行时基线「r28l 1/2」「s28l 89/118=75%」，为 C3-6 归零验证建立基座。
* 重建口径（与 collectProjectBasicEvidence 的 getFileDetail 通道 + buildDrawingFactLock 同源）：
* ① 文件集合 = draft.sources 的 filePath 快照 ∩ KB active（已实证：KB 525 = r28l 7 + s28l 518 精确匹配）；
* ② getFileDetail(relativePath).chunks 全量 → 每 chunk 过 projectBasicFactScore>0 过滤（运行时同口径）；
* ③ processingByPath 用 inferMaterialKind 文件名判定（buildProjectMaterialProfile 主干同源）；
* ④ buildDrawingFactLock({ evidence, fileProcessingByPath }) → drawingFactPlacement(lock, markdown)。
*/
var require_tmp_c3_probe_drawing = /* @__PURE__ */ __commonJSMin((() => {
	init_kbService();
	init_drawingFactLock();
	init_projectBasicInfo();
	init_projectMaterialProfile();
	const DOCS = [{
		...
		expect: "groups=2 referenced=1"
	}, {
		name: "s28l 舒城",
		file: "/Users/pan/.customize-agent/projects/3c3f04667c69/generatedDocuments/drafts/doc-1789884273697-ea5fbe91.json",
		expect: "groups=118 referenced≈89"
	}];
	async function main() {
		const only = process.argv[2];
		const manager = getMultiProjectManager();
		for (const doc of DOCS) {
			if (only && !doc.name.includes(only)) continue;
			const wrapper = JSON.parse(readFileSync(doc.file, "utf8"));
			const layer = wrapper.draft ?? wrapper;
			const markdown = String(layer.markdown ?? wrapper.markdown ?? "");
			const projectRoot = String(layer.projectRoot || "/Users/pan/Desktop/codeing/customize-agent");
			const sources = layer.sources ?? [];
			const sourceSet = new Set(sources.map((s) => s.filePath));
			console.log(`\n===== ${doc.name} ===== md=${markdown.length} 字符 | 对照运行时（期望 ${doc.expect}）`);
			const project = await manager.getProject(projectRoot);
			const allFiles = project.listFiles();
			const files = allFiles.filter((file) => file.status === "active" && sourceSet.has(file.relativePath));
			console.log(`  索引文件 ${allFiles.length} 份（active ${files.length}；sources 快照 ${sourceSet.size}）`);
			...
			for (const file of files) {
				...
				if (!firstChunk) {
					console.log("  chunk 结构首验:", Object.keys(detail.chunks[0] || {}).join(","));
					firstChunk = true;
				}
			...
			console.log(`  kind=drawing 文件 ${drawingFiles}；chunks ${chunkTotal} 保留 ${chunkKept}；evidence ${evidence.length}；detail 空 ${detailMissing}`);
			const lock = buildDrawingFactLock({
				evidence,
				fileProcessingByPath: processingByPath
			});
			const placement = lock ? drawingFactPlacement(lock, markdown) : void 0;
			console.log(`  lock: groups=${lock?.groups.length ?? 0} usable=${lock?.usableDrawings ?? 0} unusable=${lock?.unusableDrawings ?? 0} totalFacts=${lock?.totalFacts ?? 0}`);
			console.log(`  placement: referenced=${placement?.referenced.length ?? 0} unreferenced=${placement?.unreferenced.length ?? 0} rate=${placement ? placement.rate.toFixed(4) : "n/a"}`);
			if (placement?.unreferenced.length) console.log("  unref 顶部:", placement.unreferenced.slice(0, 10).map((group) => group.sourceFile.split("/").pop()).join(" | "));
			if (!lock) continue;
			for (const group of (placement?.unreferenced ?? []).slice(0, 3)) {
				console.log(`  [${group.sourceFile.split("/").pop()}] tokens=${group.tokens.length} 样例: ${group.tokens.slice(0, 8).join(",")}`);
				console.log("     事实行样例:", group.factLines.slice(0, 3).map((l) => l.slice(0, 60)).join(" || "));
			}
			const chapters = layer.chapters ?? [];
			const injectedFiles = /* @__PURE__ */ new Set();
			const perChapterCount = [];
			for (const ch of chapters) {
				const title = String(ch.title ?? "");
				if (!title) continue;
				const text = renderDrawingFactLockText(lock, title, { sections: (ch.sections ?? []).map(String) });
				let count = 0;
				for (const group of lock.groups) {
					const basename = group.sourceFile.split("/").pop() || group.sourceFile;
					if (text.includes(`${basename}｜`)) {
						injectedFiles.add(group.sourceFile);
						count += 1;
					}
				}
				perChapterCount.push(`${title.slice(0, 12)}:${count}`);
			}
			const neverInjected = lock.groups.filter((group) => !injectedFiles.has(group.sourceFile));
			console.log(`  注入覆盖：${injectedFiles.size}/${lock.groups.length} 图纸在至少一章获注入；从未注入 ${neverInjected.length} 份`);
			if (neverInjected.length) console.log("  从未注入:", neverInjected.slice(0, 12).map((group) => group.sourceFile.split("/").pop()).join(" | "));
			console.log(`  章节数 ${chapters.length}；每章注入数: ${perChapterCount.slice(0, 40).join(" ; ")}`);
			const unrefSet = new Set((placement?.unreferenced ?? []).map((group) => group.sourceFile));
			const bothGap = neverInjected.filter((group) => unrefSet.has(group.sourceFile));
			console.log(`  未引用∩从未注入 = ${bothGap.length}；未引用但曾注入 = ${(placement?.unreferenced.length ?? 0) - bothGap.length}`);
			const body = markdown.replace(/\s+/gu, "").replace(/：/gu, ":").toLowerCase();
			const tokenHolders = /* @__PURE__ */ new Map();
			for (const group of lock.groups) for (const token of group.tokens) {
				const holders = tokenHolders.get(token);
				if (holders) holders.push(group.sourceFile);
				else tokenHolders.set(token, [group.sourceFile]);
			}
			const injectedSet = new Set(injectedFiles);
			const suspicious = [];
			for (const group of lock.groups) {
				if (injectedSet.has(group.sourceFile)) continue;
				if (unrefSet.has(group.sourceFile)) continue;
				const matched = group.tokens.filter((token) => body.includes(token));
				const shared = matched.filter((token) => (tokenHolders.get(token)?.length ?? 0) >= 2);
				const uniqueMatched = matched.filter((token) => (tokenHolders.get(token)?.length ?? 0) === 1);
				suspicious.push({
					file: group.sourceFile.split("/").pop() || group.sourceFile,
					matched: matched.length,
					shared: shared.length,
					uniqueMatched: uniqueMatched.length
				});
			}
			const fullyShared = suspicious.filter((item) => item.matched > 0 && item.uniqueMatched === 0);
			console.log(`  从未注入但 referenced = ${suspicious.length}；其中「命中 token 全部为共享 token（无独有命中）」= ${fullyShared.length}`);
			for (const item of suspicious.slice(0, 8)) console.log(`   [蹭命中嫌疑] ${item.file} 命中${item.matched}(共享${item.shared}/独有${item.uniqueMatched})`);
		}
	}
	main().catch((error) => {
		console.error(error);
...
//#endregion
export default require_tmp_c3_probe_drawing();
export {};
