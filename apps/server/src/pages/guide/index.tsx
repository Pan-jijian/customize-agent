import { Card, Col, Collapse, Divider, Row, Tag, Timeline, Typography } from 'antd';
...
const stageCards = [
  ...
  },
  {
    ...
    title: '生成阶段',
    color: 'processing',
    items: ['创建模板', '知识库证据增强', '动态执行状态'],
  },
  {
    icon: <SafetyCertificateOutlined />,
    title: '交付阶段',
    color: 'green',
    items: ['检查事实和来源', '检查导出门禁', '导出正式文件'],
  },
];
...
const operationSections = [
  {
    title: '第一步：配置模型',
    goal: '让系统具备 LLM JSON 事实抽取、章节生成、自动规则检查和修复能力。',
    path: '左侧菜单 → 模型配置',
    steps: [
  ...
  {
    title: '第三步：创建提示词',
    goal: '定义每个执行阶段的模型行为，并写清系统需要自动抽取的检查规则。',
    path: '左侧菜单 → 提示词管理',
    steps: [
      '创建事实抽取提示词：要求只抽取资料中明确出现的事实，返回结构化 JSON，不允许编造。',
      '创建章节生成提示词：定义文档类型、专业语气、章节风格、引用事实和禁止事项。',
      '在总控、章节生成或角色提示词中写清工程名称、工期、质量目标、数字、表格引用、章节完整性等约束，系统运行时会自动抽取为检查规则。',
      '创建格式化提示词：定义编号、表格、分页和正式输出样式。',
      '保存后确认提示词能在提示词角色绑定下拉框中看到。',
    ],
    checks: ['事实抽取提示词已创建', '章节生成提示词已创建', '格式化提示词或角色约束已创建'],
    tips: '提示词越明确，生成结果越稳定。系统会自动抽取规则用于生成、检查和修复，建议明确写上“没有来源的数据不要写”。',
  },
  {
    title: '第四步：创建文件角色',
  ...
  {
    title: '第五步：创建提示词角色',
    goal: '告诉系统每条提示词在哪个执行阶段使用，并让系统从提示词中自动抽取检查规则。',
    path: '左侧菜单 → 角色配置 → 新建提示词角色',
    steps: [
      '填写角色名称，例如“施工组织设计事实抽取”“章节生成规则”“格式与交付规则”。',
      '选择执行类型：事实抽取、章节生成、格式化或参考；需要检查的要求直接写进对应提示词，系统运行时会自动抽取规则。',
      '绑定对应提示词。',
      '保存角色。',
      '一个执行阶段可以有多个提示词角色，后续在项目角色配置中排序。',
    ],
    checks: ['提示词角色有名称', '已选择执行类型', '已绑定提示词'],
    tips: '不需要单独维护检查类提示词；系统会从已绑定提示词中抽取禁止项、章节规则、事实规则、字数要求和导出检查规则。',
  },
  {
    ...
    steps: [
      ...
      '添加提示词角色，并设置顺序。推荐：事实抽取 → 章节生成 → 格式化 → 参考；检查规则会从已绑定提示词中自动抽取。',
      '保存项目角色配置。',
  ...
  },
  {
    ...
    steps: [
      ...
      '系统会先绑定文件角色和提示词角色，再按章节自动检索知识库证据；只有在必须指定官方资料、图纸或表格时，才需要使用高级优先证据。',
      '执行状态会根据当前模板、后台自动规范、文件角色、提示词角色、运行时规则、证据增强和章节自动生成，不是固定工程流程。',
      '当前执行节点会高亮显示，子状态会展示正在处理的章节、事实字段、文件角色、运行时规则、增强贡献或门禁规则。',
      '生成任务在后台运行，页面通过 documentId 轮询；切换页面后回到生成编辑仍可恢复状态。',
      '生成完成后，系统会用后端真实 executionStages 回填最终状态。',
      '如果导出门禁存在风险，生成完成节点会提示需要查看规则检查结果。',
    ],
  ...
  {
    title: '第九步：审查结果并导出',
    goal: '确保文档可追溯、可检查、可交付，同时能区分高风险问题和可复核 warning。',
    ...
    steps: [
      ...
      '查看“规则检查”，优先处理 error；warning 表示可导出但建议复核。',
...
export default function GuidePage() {
  ...
  return <div className="space-y-6 animateFadeIn">
    ...
    <Card title={t('guide.fullFlow')}>
      <Row gutter={[12, 12]}>
        {operationSections.map((section, index) => <Col xs={24} sm={12} lg={8} xl={6} key={section.title}>
          <Card size="small" className="h-full">
            <div className="flex items-start gap-3">
              <Tag color="blue" className="mt-1">{index + 1}</Tag>
              <div className="min-w-0">
                <Text strong>{section.title.replace(/^第.+?：/u, '')}</Text>
                <Paragraph className="mb-1 text-xs" ellipsis={{ rows: 2, tooltip: section.goal }}>{section.goal}</Paragraph>
                <Text type="secondary" className="text-xs break-words">{section.path}</Text>
              </div>
            </div>
          </Card>
        </Col>)}
      ...
      <Col xs={24} lg={12}>
        <Card title={t('guide.roleDesign')}>
          ...
          <div className="space-y-3">
            ...
            <div><Tag color="processing">config</Tag><Text strong>{t('guide.projectConfig')}</Text><Paragraph>{t('guide.projectConfigDesc')}</Paragraph></div>
          </div>
        </Card>
      </Col>
      <Col xs={24} lg={12}>
    ...
    <Card title="文件角色和提示词角色案例：它们在模板生成中到底起什么作用">
      <Row gutter={[16, 16]}>
        ...
        <Col xs={24} lg={12}>
          <Card size="small" title="案例 D：在提示词中写入交付检查要求">
            <Paragraph><Text strong>怎么创建：</Text>提示词内容写清楚必须检查的交付要求，例如“不能出现资料未提供、图片引用必须可访问、关键事实必须有来源”；执行类型选择章节生成、格式化或参考即可。</Paragraph>
            <Paragraph><Text strong>生成时的作用：</Text>系统会在运行时自动抽取这些要求，结合证据来源、事实落位和导出门禁检查文档是否可交付。</Paragraph>
            <Paragraph><Text strong>带来的提升：</Text>不需要维护单独检查类提示词，也能把人工审稿经验前置到生成流程，降低导出后才发现缺来源、缺图片、格式不合格的概率。</Paragraph>
          </Card>
        </Col>
      </Row>
    </Card>

    <Card title="运行时规则自动抽取说明">
      <Row gutter={[16, 16]}>
        <Col xs={24} md={8}><Card size="small" title="用户只维护业务提示词"><Paragraph>系统不要求用户单独创建检查类提示词。用户只需要在总控、章节生成、格式化或参考提示词中写清“必须怎么写、不能写什么、字数和章节要求”。</Paragraph></Card></Col>
        <Col xs={24} md={8}><Card size="small" title="运行时自动抽取规则"><Paragraph>生成开始时，系统会从已绑定提示词中抽取禁止词、章节规则、主体表达、商务敏感项、事实真实性、字数要求和角色关注点，并统一用于生成、检查和修复。</Paragraph></Card></Col>
        <Col xs={24} md={8}><Card size="small" title="执行状态可追踪"><Paragraph>生成状态会展示提示词规则执行摘要，方便确认用户写入的约束是否被识别并参与本次生成流程。</Paragraph></Card></Col>
      </Row>
    </Card>

    <Card title="联网增强说明">
      <Row gutter={[16, 16]}>
        <Col xs={24} md={8}><Card size="small" title="默认关闭"><Paragraph>联网增强在模型配置页手动开启。开启后，系统会受控检索公开规范、政策和通用技术资料，用于增强措施准确性。</Paragraph></Card></Col>
        <Col xs={24} md={8}><Card size="small" title="不覆盖项目事实"><Paragraph>联网资料不会用于生成或修改项目名称、地点、规模、工期、质量标准、工程量、材料规格、商务数据等项目事实。</Paragraph></Card></Col>
        <Col xs={24} md={8}><Card size="small" title="有报告和门禁"><Paragraph>生成状态会展示联网增强报告；导出前会检查并清理联网过程性话术，避免正式正文出现搜索过程描述。</Paragraph></Card></Col>
      </Row>
    </Card>

    <Card title="我的数据和门禁规则说明">
      <Row gutter={[16, 16]}>
        <Col xs={24} md={8}><Card size="small" title="使用自己的数据"><Paragraph>文件、提示词、角色配置和规范包都来自用户自己创建或上传的内容，生成流程会按当前项目配置读取，并自动抽取检查规则参与生成和导出门禁。</Paragraph></Card></Col>
        <Col xs={24} md={8}><Card size="small" title="自定义门禁类型会执行"><Paragraph>门禁规则由后台根据模板、提示词和角色绑定自动形成，并与系统门禁一起参与规则检查，不需要用户手动维护单独检查类提示词。</Paragraph></Card></Col>
    ...
    <Card title="知识库证据和增强贡献说明">
      <Row gutter={[16, 16]}>
        <Col xs={24} md={12}><Card size="small" title="自动检索为主"><Paragraph>模板章节的查询词、章节目的和必需事实会驱动系统自动检索知识库证据。高级优先证据只是兜底能力，用于必须指定官方资料、图纸或表格的场景，不是日常必填项。</Paragraph></Card></Col>
        <Col xs={24} md={12}><Card size="small" title="增强贡献可追踪"><Paragraph>生成流程会展示知识库证据、人工确认/固定证据和自动检索证据的数量，帮助判断本次增强是否真的参与生成，而不是增加用户维护成本。</Paragraph></Card></Col>
      </Row>
    </Card>

    <Card title="动态 schema、证据和执行状态说明">
      <Row gutter={[16, 16]}>
        <Col xs={24} md={8}><Card size="small" title="动态事实 schema"><Paragraph>系统不会写死工程字段。后台自动规范会结合模板章节 requiredFacts、角色绑定和提示词形成抽取 schema，并驱动事实抽取、冲突检测、规则检查和导出门禁。</Paragraph></Card></Col>
    ...
    <Card title="生成资源和知识库的关系">
      <Row gutter={[16, 16]}>
        <Col xs={24} md={12}>
          <Card size="small" title="生成资源不进入知识库">
            <Paragraph>模板运行生成的文档、图片、封面或后处理资源只会登记到生成资源管理页，不会写入 knowledgeBase，也不会建立知识库索引。</Paragraph>
            <Paragraph>这样可以避免模型产物污染事实来源；需要作为资料复用时，应由用户明确上传到知识库。</Paragraph>
          </Card>
        </Col>
        <Col xs={24} md={12}>
    ...
    <Card title="文件角色处理类型大白话说明">
      <Row gutter={[16, 16]}>
        ...
        <Col xs={24} md={12}><Card size="small" title="规范文件"><Paragraph>用来作为专业依据。适合放国家规范、行业标准、企业标准、攻略结构规范、质量标准。生成、规则检查和导出门禁会把它当成依据。</Paragraph></Card></Col>
    ...
    <Card title="常见问题和处理建议">
      <Collapse items={[
        ...
        { key: 'asset-index', label: '生成资源会如何进入知识库？', children: <Paragraph>生成资源不会自动或手动写入知识库，也不会建立知识库索引，避免生成内容再次被当作事实证据召回。确需复用时，请由用户明确上传到知识库。</Paragraph> },
