...
export default function DocumentSpecsPage() {
  ...
  const factOptions = (Form.useWatch('factFields', form) ?? [])
    .filter((field: { id?: string; name?: string }) => field.id && field.name)
    .map((field: { id: string; name: string }) => ({ label: `${field.name}（${field.id}）`, value: field.id }));

  const defaultFactFields = (): DocumentSpecPackage['factFields'] => [
  ...
  return <div className="space-y-6 animateFadeIn">
    ...
    <Card title={t('specs.list')}>
      <List
        ...
        renderItem={(spec, index) => <List.Item actions={[<Button key="edit" type="text" icon={<EditOutlined />} onClick={() => openEditor(spec)} />, <Popconfirm key="delete" title={t('common.confirm')} disabled={spec.builtIn} onConfirm={() => { void remove(spec.id); }}><Button type="text" danger icon={<DeleteOutlined />} disabled={spec.builtIn} /></Popconfirm>]}>
          <List.Item.Meta title={<Space><Tag>序号 {index + 1}</Tag><span>{spec.name}</span>{spec.builtIn && <Tag color="gold">{t('documents.builtIn')}</Tag>}<Tag>{spec.id}</Tag></Space>} description={`${spec.description || '-'} · ${t('specs.facts')}: ${spec.factFields.length} · ${t('specs.chapters')}: ${spec.chapterRules.length} · ${t('specs.gates')}: ${spec.gateRules.length}`} />
        </List.Item>}
      />
    </Card>
