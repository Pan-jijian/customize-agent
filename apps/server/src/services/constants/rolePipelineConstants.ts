...
/** 章节生成后必须阻断通过的问题。 */
export const BLOCKING_CHAPTER_ISSUE_RE = /正文缺少章节标题|缺少规划小节|空小节|小节只有标题|只有标题或表格无正文|规划小节正文过短|正文篇幅明显低于目标|后台流程话术|重复 token|退化输出/u;

/** 可通过局部 LLM 修复的质量问题。 */
export const REPAIRABLE_QUALITY_ISSUE_RE = /阻断|错误|正文缺少章节标题|缺少规划小节|空小节|小节只有标题|只有标题或表格无正文|规划小节正文过短|缺少必要的正式表格|事实一致性|其他项目|项目编号|项目名称|后台流程|提示词|结构化精确参数使用不足|正文未体现结构化数据资料|正文未体现设计\/方案\/说明类资料|图片|重复 token|退化输出/u;

/** 修复问题分类规则，按顺序匹配。 */
export const QUALITY_REPAIR_TYPE_RULES: Array<{ type: QualityRepairType; pattern: RegExp }> = [
  { type: 'missing_structure', pattern: /缺少规划小节|二级小节少于|三级小节|目录|章节|结构/u },
