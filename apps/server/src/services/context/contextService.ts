...
function tokenizeContextQuery(query: string) {
  const terms: string[] = [];
...
function buildContextSearchParams(terms: string[]) {
  ...
  for (const term of terms) {
    clauses.push('(content LIKE ? OR context LIKE ?)');
    params.push(`%${term}%`, `%${term}%`);
  }
  return { clauses: clauses.join(' OR '), params };
}

function mapRowsToEntries(rows: unknown[]) {
  const entries: ContextEntry[] = [];
  for (const row of rows) entries.push(rowToEntry(row as Record<string, unknown>));
  return entries;
}
