...
export function chapterReadinessIssues(readiness: ChapterReadiness[]): ValidationIssue[] {
  ...
  for (const item of readiness) {
    ...
    else if (item.readinessRate < 0.5) issues.push({ level: 'warning', message: `章节事实覆盖率较低：${item.title} ${Math.round(item.readinessRate * 100)}%`, suggestion: '建议补充章节 requiredFacts 或绑定材料证据。' });
  }
  return issues;
}
