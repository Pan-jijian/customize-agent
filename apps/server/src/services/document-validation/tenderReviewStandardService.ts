import { getMultiProjectManager } from '../knowledge/kbService';
import type { DocumentTemplateChapter } from '../document-workflow/types';
...
function isValidReviewChapterTitle(title: string) {
  ...
  if (/^(必须|不得|禁止|需要|请|应|输出|返回|使用|格式|示例|为确保|完全满足|具体)/u.test(clean)) return false;
  if (/(评标委员会|完全满足|全面梳理|全面兑现|技术保障|管理目标|具体项目概况)/u.test(clean)) return false;
...
function sectionCore(title: string) {
  return normalizeTitle(title)
    .replace(/^(?:对|关于)/u, '')
    .replace(/(?:的)?(?:方案|措施|计划|体系|管理|控制|保障|要求|安排|内容)$/u, '')
...
function chapterFromTitle(title: string, index: number, defaults: ReviewChapterSectionDefaults): DocumentTemplateChapter {
  ...
  return {
    ...
    title: normalized,
    purpose: `依据配置的详细评审标准中“${normalized}”要求编写，必须响应评审要点并结合当前项目资料展开。`,
    queries: [normalized, ...sections, '详细评审标准', '评审标准'],
    requiredFacts: ['项目名称', '实施范围', '周期要求', '质量要求', '安全要求'],
    sections,
...
function extractChapterTitles(text: string) {
  ...
  const fallbackStartIndex = lines.findIndex(line => /详细评审标准|技术文件详细评审标准/iu.test(line));
