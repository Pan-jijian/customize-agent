...
export function workPackageSkeletonPrompt(projectContext: string, evidence: DocumentEvidence[], minCount = 3, namesOverride?: string[]): string {
  ...
  return [
    ...
    '要素写法：作业对象与工程量（作业对象与部位、工程量或规模、材料设备规格型号，数量类数值优先取工程量清单数据）；工序顺序（先后顺序清晰，相邻小节不得同句式开头，同一句式全文不得反复使用，禁止以固定句模复读）；施工方法（工艺做法、工艺参数（数值+单位）、验收检测与记录闭环）。',
    // C4 D7：示例尾部原为「检测合格后形成记录归档闭环」——固定闭环句式被 Writer 集中抄写（r28l/s28l 实测 11/28 处），
    // 改为中性归档表述；并补警示：句式同样不得照抄（示例即模板化源头）
    '融合叙述示例（仅为形式示意：数值必须替换为本项目绑定资料中的真实数值，句式不得照抄、须按本节资料重新组织）：\n本工程室外道排范围覆盖园区内雨水、污水管网及检查井，主要工程量 HDPE 双壁波纹管 DN300 约 1200m、检查井 45 座，管材环刚度 SN8。施工按测量放线、沟槽开挖、管道基础、管道铺设、闭水试验、分层回填、压实度检测、验收的顺序组织；沟槽机械开挖配合人工清底，槽底标高偏差控制在±20mm以内，接口采用承插式橡胶圈连接，回填每层虚铺厚度不超过300mm、压实度不低于95%，压实度检测结果当日整理归档。',
...
export function keySectionWritingRequirement(sectionTitle: string) {
  ...
  if (/项目主要施工内容/u.test(sectionTitle)) return [
    ...
    '要素写法：作业对象与工程量写对象范围、工程量或规模、材料设备规格、施工部位；工序顺序必须有明确的顺序表达（相邻小节不得同句式开头，同一句式全文不得反复使用，禁止以固定句模复读）；施工方法写工艺做法、穿插组织、工艺参数、质量验收、检测复试、资料闭环。',
    '数值来源优先级：工程量、材料规格、设备型号等数量类数值优先取工程量清单数据；清单未覆盖的参数（标高、坡率、构造做法等）才取图纸数据；禁止“按设计图纸执行”“详见设计图纸”“按设计文件确定”式概括话术——必须落到具体数值或具体规范条文。',
  ...
  if (/主要分部分项工程施工方案|主要施工方法/u.test(sectionTitle)) return [
    ...
    '每个分项工程方案必须覆盖作业对象与工程量、工序顺序、施工方法三方面要素（三要素缺一不可），融入连贯段落叙述，禁止以“施工概况/工艺流程/施工方法”等结构标签充当标题或段落开头引导；工序顺序表达须先后清晰（相邻小节不得同句式开头，同一句式全文不得反复使用，禁止以固定句模复读）；数值来源优先级同主要施工内容：数量类数值优先取工程量清单，清单未覆盖才取图纸数据，禁止“按设计图纸执行”式概括话术。',
  ].join('\n');
  return '';
}
