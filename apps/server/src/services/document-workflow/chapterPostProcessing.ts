...
export function workPackageSkeletonPrompt(projectContext: string, evidence: DocumentEvidence[], minCount = 3, namesOverride?: string[]): string {
  ...
  return [
    ...
    // 4.60 I2-c：**示例整段删除**（保留形式描述）。本处原有一段完整范例段落（含 DN300 约 1200m、
    // 检查井 45 座、SN8 等写实数值），虽标注「句式不得照抄」，但示例即模板化源头——C4 D7 已实测
    // 其尾部闭环句被集中抄写 11/28 处；整段范例还会让不同项目的「室外道排」小节长成同一篇
    //（连数值位置都一样），是跨文档雷同与事实漂移的共同上游。形式要求用文字描述即可，不给范例。
    '融合叙述要求（**只描述形式，不给范例**）：三要素必须写成连贯段落——先交代作业对象与范围（部位/单体 + 工程量或规格），再按施工先后把工序串起来叙述，每个工序夹带对应的工艺参数（数值+单位）与验收检测做法；不得分条罗列成要素清单，不得先写「作业对象：」「施工方法：」式标签再补内容。',
