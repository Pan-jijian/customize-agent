...
const CHAPTER_FOCUS_RULES: Array<{ pattern: RegExp; goal: string; mustCover: string[] }> = [
  { pattern: /概况|总体|理解|说明|编制/u, goal: '以项目资料已确认的事实卡片展开工程概况与总体理解，不得空泛概述', mustCover: ['工程名称、建设地点、建设规模、计划工期、质量标准等资料已确认事实', '现场条件与招标范围边界', '编制依据与适用范围', 'B5 专业工程清单：资料识别的专业工程逐个展开（不少于 5 个，每个专业工程写明作业对象与工程量、工序顺序、施工方法三方面要素，落到工程量与工艺参数）'] },
  ...
  { pattern: /安全|危大|风险/u, goal: '覆盖风险识别、危大工程专项方案、检查整改与应急响应', mustCover: ['危险源辨识与风险分级', '危大工程清单与专项方案', '安全交底与隐患排查闭环', 'B5 应急预案：应急组织架构、应急物资储备清单、专项预案与演练计划（演练频次量化，如每半年 1 次）'] },
  { pattern: /资源|材料|设备|劳动力|人材机/u, goal: '说明资源配置依据、进场验收与保管调配', mustCover: ['机械设备投入计划', '分阶段劳动力计划', '材料进场计划与验收'] },
  { pattern: /文明|绿色|环保|扬尘|噪声/u, goal: '覆盖扬尘噪声管控、四节一环保与智慧监测', mustCover: ['扬尘噪声分时段管控', '四节一环保措施', '监测预警与台账', 'B5 四节一环保量化指标：节水（非传统水源利用率/用水量控制）、节材（材料损耗率/废弃物回收率）、节地（土方平衡率）、节能（能耗指标）各至少 1 项带单位量化指标，附录八基准对照项逐项量化响应'] },
...
export function buildWritingTaskBrief(input: {
  ...
  const globalWritingFocus = [
    ...
    '正文每 1000 字至少落位 6 处带单位量化工艺参数（mm/MPa/养护天数/间距/压实度等），施工流程与施工方法须有明确的工序顺序表达（形式由模型自然选择、不做统一要求：顺序词叙述、编号步骤、有序/无序列表或箭头链均可），每个分部分项方案至少 1 处 3 环节以上工序顺序表达',
    '专业工程类小节必须覆盖作业对象与工程量、工序顺序、施工方法三方面要素（呈现形式不限，段落式叙述或带标签分点均可），小节标题用专业工程正式名称（如"拆除工程"），禁止使用"工作包"等后台概念命名',
