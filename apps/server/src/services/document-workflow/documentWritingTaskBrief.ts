...
const CHAPTER_FOCUS_RULES: Array<{ pattern: RegExp; goal: string; mustCover: string[] }> = [
  ...
  { pattern: /安全|危大|风险/u, goal: '覆盖风险识别、危大工程专项方案、检查整改与应急响应', mustCover: ['危险源辨识与风险分级', '危大工程辨识清单逐项完整（按项目实际情况覆盖基坑支护与降水、模板支撑、脚手架、起重吊装、吊篮、拆除等适用项，逐项写明辨识依据，不得遗漏）', '危大工程专项方案编制与审批程序', '安全交底与隐患排查闭环', 'B5 应急预案：应急组织架构、应急物资储备清单、专项预案与演练计划（演练频次量化，如每半年 1 次）'] },
  { pattern: /资源|材料|设备|劳动力|人材机/u, goal: '说明资源配置依据、进场验收与保管调配', mustCover: ['机械设备投入计划', '分阶段劳动力计划', '材料进场计划与验收'] },
  { pattern: /文明|绿色|环保|扬尘|噪声/u, goal: '覆盖扬尘噪声管控、四节一环保与智慧监测', mustCover: ['扬尘治理六个百分百逐项落位（施工工地周边 100% 围挡、物料堆放 100% 覆盖、出入车辆 100% 冲洗、施工现场地面 100% 硬化、拆迁工地 100% 湿法作业、渣土车辆 100% 密闭运输；不涉及拆迁的写明豁免理由）', '扬尘噪声分时段管控', '四节一环保措施', '监测预警与台账', 'B5 四节一环保量化指标：节水（非传统水源利用率/用水量控制）、节材（材料损耗率/废弃物回收率）、节地（土方平衡率）、节能（能耗指标）各至少 1 项带单位量化指标，附录八基准对照项逐项量化响应'] },
  ...
  { pattern: /工资|劳务|实名/u, goal: '覆盖劳务实名制与农民工工资保障闭环', mustCover: ['劳务实名制管理', '工资专用账户与银行代发', '考勤与工资支付台账', '工伤保险办理与参保信息管理（作业人员工伤保险按项目参保、参保信息纳入实名制管理、工伤事故申报处置流程）'] },
];
...
export function buildWritingTaskBrief(input: {
  ...
  const globalWritingFocus = [
    ...
    '正文每 1000 字至少落位 6 处带单位量化工艺参数（mm/MPa/养护天数/间距/压实度等），施工流程与施工方法须有明确的工序顺序表达（形式按小节序号轮换使用：顺序词叙述、编号步骤、有序列表、箭头链，禁止相邻小节同一形式、禁止通篇同一形式），每个分部分项方案至少 1 处 3 环节以上工序顺序表达',
