...
/**
 * 重难点「归因 + 量化目标」双达标要求（单源）。
 * 与检测端 tenderBidChecks 同源：每条难点须同时满足「归因（语义：分析成因与风险来源）」
 * 与「量化目标（结构：QUANTIFIED_TARGET_RE = 数值+单位/频次）」，双达标率 ≥50% 才不判模板化。
 * 实测缺陷：丰乐镇成稿的重难点以表格呈现（每行 = 一条），行内只有定性描述、**无任何数值** →
 * 双达标 0%（线 50%），节点标红。重难点内容既可能落在「重点难点分析」章、也可能落在概况章，
 * 故两条焦点规则都引用本常量，避免两处口径漂移。
 */
const DIFFICULTY_QUANTIFIED_REQUIREMENT =
  ...
  + '重难点以表格呈现时按行逐条判定（检测口径：归因表述 + 数值单位结构，双达标率需 ≥50%）';
