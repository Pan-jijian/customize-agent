...
const CHAPTER_FOCUS_RULES: Array<{ pattern: RegExp; goal: string; mustCover: string[] }> = [
  ...
  { pattern: /主要施工内容/u, goal: '按专业工程展开施工概况、施工流程、施工方法三段式卡片，落到工程量与工艺参数', mustCover: ['资料识别的专业工程（不少于5个）', '每个专业工程的施工概况/施工流程/施工方法', '工程量、材料设备规格、检测验收与资料闭环'] },
  // 分部分项专属规则：历史上“主要分部分项工程施工方案”章 12 条全部不匹配落默认 goal（概略根因之一），
  // 与 writingSpec 专项提示词同口径（每个分项方案三段式+4 参数+工序顺序表达）
  { pattern: /主要分部分项工程施工方案|主要施工方法/u, goal: '按分项工程方案展开施工概况、工艺流程、施工方法三段式，落到工艺参数与工序顺序', mustCover: ['资料明确的专业工程范围（逐项展开分项工程方案）', '每个分项方案的施工概况/工艺流程/施工方法', '每个分项方案不少于4个工艺参数与至少1处3环节以上工序顺序表达', '工艺参数来自绑定材料或行业通用规范值，不得编造'] },
...
export function buildWritingTaskBrief(input: {
  ...
  const globalWritingFocus = [
    ...
    '专业工程类小节按"施工概况/施工流程/施工方法"三段式展开，小节标题用专业工程正式名称（如"拆除工程"），禁止使用"工作包"等后台概念命名',
