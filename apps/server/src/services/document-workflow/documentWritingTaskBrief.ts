...
const CHAPTER_FOCUS_RULES: Array<{ pattern: RegExp; goal: string; mustCover: string[] }> = [  { pattern: /概况|总体|理解|说明|编制/u, goal: '以项目资料已确认的事实卡片展开工程概况与总体理解，不得空泛概述', mustCover: ['工程名称、建设地点、建设规模、计划工期、质量标准等资料已确认事实', '现场条件与招标范围边界', '编制依据与适用范围（按五类逐项列全：招标文件及补疑补遗、国家法律法规、国家/行业现行规范标准、地方法规规章、企业管理体系；地方法规规章必须结合工程所在地属地的现行地方性法规与政府规章，不得只写笼统一句话）', 'B5 专业工程清单：资料识别的专业工程逐个展开（不少于 5 个，每个专业工程写明作业对象与工程量、工序顺序、施工方法三方面要素，落到工程量与工艺参数）', `重点难点分析（本类章普遍承载该内容）：${DIFFICULTY_QUANTIFIED_REQUIREMENT}`] },
  ...
  { pattern: /竣工|验收|移交|保修/u, goal: '覆盖竣工清理、验收移交与保修响应', mustCover: ['竣工清理与垃圾外运', '缺陷修补与复验', '验收移交与保修响应'] },
...
export function buildWritingTaskBrief(input: {
  ...
  const globalWritingFocus = [
    ...
    '措施类内容必须可核查：每项措施写清执行主体（谁做）、动作与量化标准（做什么、达标线是多少）、检查频次（多久一次）与问题处置（不达标怎么处理、由谁验证、留下什么记录）；**闭环要用实质动作表达**（复测、送检、旁站、检验批验收、实测实量、资料归档等），禁止以“复查销项”“整改销项”“复查确认”一类无执行主体、无依据、无记录的零信息尾词收句；同一收尾表达全文不得超过 4 次；相邻小节不得同句式开头',
    '措施类段落必须落位完整要素链——方案（专项施工方案/技术措施）、流程（施工工序/工艺流程）、执行主体（岗位）、检查频次（每周/每日不少于 N 次）、验收与处置（检测/试验/验收/整改/复验的具体做法）——五类要素按业务实质写清即可，**不要求出现特定字词**，也不要求逐段齐备（避免为凑要素而填充套话）',
