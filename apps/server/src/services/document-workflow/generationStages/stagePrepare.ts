...
export async function stagePrepare(session: GenerationSession): Promise<void> {
  ...
  session.prepare.generationWritingConstraintsPrompt = ['【全文写作硬约束（每一章生成时必须遵守，违反即评审失分）】',
    ...
    '2. 禁止商务内容（全文零商务句）：暂列金额、暂估价、综合单价、清单合价、税率、投标报价、付款、进度款、工程款、结算、保证金、违约金、预付款、保函等商务数据，以及“按合同约定执行”式付款/保证金/结算/违约商务声明句，一律不得写入施工组织设计正文——商务与合同条款的响应属商务标内容，技术标正文不得出现任何商务内容，商务口径只允许出现在项目信息表中。',
    ...
    '3b. 编制依据小节必须列出具体法规名称及文号：国家法律法规条目列出《中华人民共和国建筑法》（主席令第91号公布，2019年修正）、《建设工程质量管理条例》（国务院令第279号公布，2019年修订）等具体名称及文号；地方性法规与政府规章条目按工程所在地列出具体名称（省/市级现行有效法规，如"XX省建筑市场管理条例""XX市城市绿化管理条例"形态，名称须为工程所在地真实法规）；规范标准条目列出《建筑工程施工质量验收统一标准》（GB 50300-2013）等具体编号；招标文件原文引用的法规按其原文列出。不得只写“国家现行法律、行政法规”“现行规范标准”“地方法规规章”等类别话术。',
    // 4.60 I2-c：长同形列举的**跳字残缺**预防（263 份语料实测 13 份命中：编制依据段一次列 20+ 条
    // 同形「《X工程施工质量验收规范》（GB NNNNN-YYYY）」，模型在该形态上自吞噬中间段，产出
    // 「《建筑给水排水及采暖50242-2002）」这类残缺引用——同形串越长越密越易发）。修法不在下游补救，
    // 而在源头把长同形串**拆行**：每条独立成行可把单次复制跨度压到一条之内，退化概率大幅下降。
    '3c. 编制依据的规范标准条目**逐条独立成行**（每行一条，行首统一用「- 」），不得把多条挤进同一长句用「、」连写——长同形列举易引发跳字残缺（如把「《建筑给水排水及采暖工程施工质量验收规范》（GB 50242-2002）」写成「《建筑给水排水及采暖50242-2002）」），写完后逐条自检书名号与括号是否成对。',
    '4. 禁止跨章复述概况：除“工程概况/项目概况”章节外，其余章节不得以“本项目为/本工程为”开头整段复述项目总体概况，应直接展开本章主题内容。',
    '5. 禁止后台内部话术与条款响应元语言：不得出现“工作包”“WRITER_MISSING_SECTION”“已确认资料”等系统内部术语与兜底话术，一律改写为正式施工组织设计表述；不得书写“招标要求响应（前附表响应条款）”“按上述条款执行”“按招标文件约定/要求”等条款响应条幅或调用式元语言（评标人视角即编制模板痕迹）——技术性前附表条款一律以投标人口吻的正式技术表述落位各章节；商务性前附表条款（付款/保证金/结算等）不在技术标响应。',
    ...
    '9. 禁止自行分配清单规格拆分数值：清单给出规格-数量拆分（如一般路灯 100W 109套、120W 9套）时必须逐项照抄原值，不得改动或重新分配；清单只给总量的不得自行拆分（“其中 A 套、B 套”式自拆即数据错误）。同一规格-数量拆分在全文各章必须一致。',
    // 4.40 行文完整性（4.39 实机受阻「句尾截断/标点断裂/成对符号/重复行」四类根因治理：无标点悬空
    // 行尾与半句叙事均为生成侧即成，事后修补链逐轮零星收敛且可再引入——前置到写作硬约束从源头杜绝）
    '10. 行文完整性：叙述句必须以句末标点（。；！？）收尾，禁止无标点悬空行尾——不得出现“提示语或主语残段（如“……施工”）＋换行＋列表/编号”式断句；括号、引号、书名号必须成对闭合；禁止连续重复标点（如“。。”“，，”）；禁止与相邻行逐字相同的复读行；正文内的分行编号必须从 1 起连续，不得跳号、重号。',
    // D-T4 ① 选材纪律（r28f #4/#20 归因）：招标资格承诺句与项目经理变更/开工令签发程序被逐字转述
    // 进技术标正文——此类信息属商务与合同管理范畴（商务册职责），写作时源头过滤（正文清洗通道只兜底实测形态）
    '11. 禁止转述合同管理程序条款：不得抄录或转述招标文件中的投标资格承诺原文（“拟派项目经理目前无在岗项目”“承诺从其他项目变更至本项目”类）与合同履约管理程序（开工令签发程序、项目经理变更批准程序、人员请假/更换/离场批准程序、保险投保办理程序、分包审批程序、资料报送审批备案期限、违约责任与罚款明细、工程照管责任起止）；此类内容属商务标与合同管理范畴，技术标只写技术方案与现场管理措施——如项目经理到岗管理只写“项目经理按投标承诺到岗履职，全面负责本工程现场管理”，不展开变更与签发程序细节。'].join('\n');
