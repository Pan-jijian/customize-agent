...
export async function stagePrepare(session: GenerationSession): Promise<void> {
  ...
  session.prepare.generationWritingConstraintsPrompt = ['【全文写作硬约束（每一章生成时必须遵守，违反即评审失分）】',
    ...
    '5. 禁止后台内部话术与条款响应元语言：不得出现“工作包”“WRITER_MISSING_SECTION”“已确认资料”等系统内部术语与兜底话术，一律改写为正式施工组织设计表述；不得书写“招标要求响应（前附表响应条款）”“按上述条款执行”“按招标文件约定/要求”等条款响应条幅或调用式元语言（评标人视角即编制模板痕迹）——前附表条款一律以投标人口吻的正式表述落位各章节，如“本工程履约保证金按合同约定的金额、提交期限与退还时限执行”。',
