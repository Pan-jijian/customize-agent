 ...
 * - 'clarification-amendment' → buildStandardFinalValidationIssues det('caliber-consistency') 容器内
 *   答疑技术性修正检测（clarificationAmendments.clarificationAmendmentIssues）实时重跑——
 *   修复轮改写正文后必须按最新 finalMarkdown 重判，否则已落位/已删残留仍带旧警告进交付。
...
export const SNAPSHOT_DETECTOR_IDS: ReadonlySet<string> = new Set([
  ...
  'clarification-amendment', // clarificationAmendments.clarificationAmendmentIssues（“被取代形态残留 / 答疑修正未落位”），
  // 账本自 session.planning 随会话下传、每轮重算按最新 finalMarkdown 重跑（读写同源）
]);
