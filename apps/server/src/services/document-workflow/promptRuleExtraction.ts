...
export function professionalSectionTaskCard(chapterTitle: string, sectionTitle: string) {
  ...
  const points = [
    /项目主要施工内容/u.test(joined) ? '每个施工工作包（#### 工作包名）必须逐包覆盖三方面要素：①作业对象与工程量（本项目部位、规模、系统边界，工程量数值与工程量清单汇总值一致）、②工序顺序（先后清晰，至少 1 处不少于 4 个环节的工序顺序表达）、③施工方法（工具机具、工艺参数、验收闭环）；只写流程不写概况、或只列方法不写流程的工作包会被退回重写；每个专业工程单独一个小节，不得把所有专业工程合并；不得以表格代替叙述（正文表格须来自系统表格计划，不得自设），严禁出现“分部小计/本页小计/合计/按实/暂估/综合单价/措施项目费/规费/税金”等清单计价表内部口径词（“措施项目”作为专业工程类别名称可用，不得与“清单/计价/费”连用）。' : '',
    ...
    /施工|工艺|技术|方案/u.test(joined) ? '必须写清施工准备、工艺流程、关键控制点、验收要求和资料依据；每个分项工程方案必须落位至少 4 个工艺参数（mm、MPa、间距、偏差、坡度、养护天数、试验压力、搭接长度等），参数来自绑定资料或行业通用规范值，不得编造；纯设备配置型内容必须写型号、规格、容量、数量参数。工序顺序表达：工艺流程必须有明确的工序顺序表达（系统已为各小节指定表达方式；相邻小节不得同句式开头，同一句式全文不得反复使用，禁止以固定句模复读），每个含方法叙述的三级小节方法段正文至少 1 处不少于 4 个环节的工序顺序表达，不得只在单独的流程行出现。' : '',
    /流程|顺序|工序|穿插|闭环|整改|演练|转运|三检|隐蔽|排查/u.test(joined) ? '流程/顺序型叙述必须有明确的工序顺序表达（相邻小节不得同句式开头，同一句式全文不得反复使用，禁止以固定句模复读），每条序列不少于 3 个环节，把纯文字流程描述改写成分步清晰、衔接自然的表达，正文中至少 2 处工序顺序表达。' : '',
  ].filter(Boolean);
  const arrowChainPoint = points.find(point => point.includes('工序顺序表达'));
  return ['【小节专业任务卡】', `任务对象：${sectionTitle}`, ...(points.length ? points : ['必须结合本项目资料明确事实说明对象范围、实施方法、控制要点、验收要求和资料闭环，避免泛化套话。']), ...(arrowChainPoint ? ['工序顺序表达是硬性内容要求：正文成稿必须实际出现工序顺序表达（系统已为各小节指定表达方式；相邻小节不得同句式开头，同一句式全文不得反复使用，禁止以固定句模复读），评审将核验，未达标会被退回重写。'] : []), '禁止套话：不得使用“本小节围绕……展开”“结合绑定项目资料、施工组织安排和现场实施条件”“交底覆盖率按100%控制”等模板化开篇；不得只写“加强管理、严格把控、确保质量”式口号；每个三级小节必须写与本节标题对应的专属内容，不得与其他小节内容相同或近似；每个句子必须携带至少一个信息载体（数值、部位、材料、工序、岗位、时限其一），无信息载体的空泛句不得出现——判断标准是句中不含数值、部位、材料、工序、岗位、时限、记录中的任何一项，而不是句中不含某个固定说法。'].join('\n');
}
