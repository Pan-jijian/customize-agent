...
export function professionalSectionTaskCard(chapterTitle: string, sectionTitle: string) {
  ...
  const points = [
    ...
    /施工|工艺|技术|方案/u.test(joined) ? '必须写清施工准备、工艺流程、关键控制点、验收要求和资料依据；每个分项工程方案必须落位至少 4 个工艺参数（mm、MPa、间距、偏差、坡度、养护天数、试验压力、搭接长度等），参数来自绑定资料或行业通用规范值，不得编造；纯设备配置型内容必须写型号、规格、容量、数量参数。工序链写法：工艺流程必须用“→”串联关键工序，方法叙述中的连续工序序列同样用“→”串联（如“基层清理→放线定位→分层摊铺→碾压→压实度检测→验收”），每个含方法叙述的三级小节方法段正文至少 1 条不少于 4 个环节的箭头工序链，不得只把箭头局限在单独的流程行。' : '',
    /流程|顺序|工序|穿插|闭环|整改|演练|转运|三检|隐蔽|排查/u.test(joined) ? '流程/顺序型叙述必须用“→”串联关键环节形成工序链（每条链不少于 3 个环节），把纯文字流程描述改写成箭头链，如“发现问题→登记建档→原因分析→整改落实→复查销号”，正文中至少 2 条箭头链。' : '',
  ].filter(Boolean);
  const arrowChainPoint = points.find(point => point.includes('箭头工序链')) || points.find(point => point.includes('箭头链'));
  return ['【小节专业任务卡】', `任务对象：${sectionTitle}`, ...(points.length ? points : ['必须结合本项目资料明确事实说明对象范围、实施方法、控制要点、验收要求和资料闭环，避免泛化套话。']), ...(arrowChainPoint ? ['箭头链是硬性格式要求：正文成稿必须实际出现“→”字符串联的工序链，评审将按“→”出现次数核验，未达标会被退回重写。'] : []), '禁止套话：不得使用“本小节围绕……展开”“结合绑定项目资料、施工组织安排和现场实施条件”“交底覆盖率按100%控制”等模板化开篇；不得只写“加强管理、严格把控、确保质量”式口号；每个三级小节必须写与本节标题对应的专属内容，不得与其他小节内容相同或近似。'].join('\n');
}
