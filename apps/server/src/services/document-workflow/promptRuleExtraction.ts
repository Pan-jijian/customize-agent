...
export function professionalSectionTaskCard(chapterTitle: string, sectionTitle: string) {
  ...
  return ['【小节专业任务卡】', `任务对象：${sectionTitle}`, ...(points.length ? points : ['必须结合本项目资料明确事实说明对象范围、实施方法、控制要点、验收要求和资料闭环，避免泛化套话。']), ...(arrowChainPoint ? ['工序顺序表达是硬性内容要求：正文成稿必须实际出现工序顺序表达（系统已为各小节指定表达方式；相邻小节不得同句式开头，同一句式全文不得反复使用，禁止以固定句模复读），评审将核验，未达标会被退回重写。'] : []), '禁止套话：不得使用“本小节围绕……展开”“结合绑定项目资料、施工组织安排和现场实施条件”“交底覆盖率按100%控制”等模板化开篇；不得只写“加强管理、严格把控、确保质量”式口号；每个三级小节必须写与本节标题对应的专属内容，不得与其他小节内容相同或近似；每个句子必须携带至少一个信息载体（数值、部位、材料、工序、岗位、时限其一），无信息载体的空泛句不得出现（如“加强过程管控”应改为“施工员每日检查不少于2次，发现问题当日整改销项”）。'].join('\n');
}
