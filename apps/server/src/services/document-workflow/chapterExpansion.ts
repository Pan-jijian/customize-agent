...
function isNonConstructionEvidenceSentence(sentence: string) {
  // 窄过滤（模块1b）：只排除投标程序句，放行前附表实质条款句。
  // 历史缺陷：裸「投标人须知」误伤「投标人须知前附表规定：确保黄山杯」类实质条款句；裸「保证金」误伤
  // 质量/履约保证金句（施组响应项）；「履约保证金」是履约担保条款（施组必须响应）却被排除。
  return /资料参数行摘要|房建市政施工评定分离招标示范文本|我方已仔细研究|中标通知书|签订合同|投标函|招标公告|开标时间|开标地点|评标委员会|评标办法|投标保证金|保证金账户|电子交易系统|公共资源交易|监管部门|专用账户监管协议书|资金托管专用账号/u.test(sentence)
    || /^#+\s*/u.test(sentence)
    || /^（?\d+）/u.test(sentence);
}
