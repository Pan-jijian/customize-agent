...
import { extractMarkdownTableCandidates, finalizeTableCaptions, matchMissingTablePlans } from './constructionOrgTablePlan';
...
export const MARKDOWN_TABLE_FORMAT_RULES = [
  ...
  '表格数据完整性硬约束：数据行每一列都必须有具体数据值，不得出现空单元格；不得用“—/若干/约/待定/暂无/不适用”等占位或模糊表达代替具体数据（**仅**合计/小计/总计/累计行的“—”为不适用语义，可保留）。**规格型号列必须写具体型号或规格**：机械设备由投标人自行选型时，选型结论本身就是必须写出的值（小型机具写型号或额定参数），不得以“—”留空；不得凭空编造型号。数据优先取自本项目资料与蓝图权威锚点；计划类数值（人数、台班、进度时间）必须原样引用蓝图推导值，不得按工程量、工期或定额自行推算另设。',
...
export const TENDER_BID_WRITING_RULES = [
  '【内容落地五要素】每项管控措施必须写全五要素：方案 + 流程 + 责任人 + 时间节点 + 验收标准；责任人落到具体岗位，检查频次量化到每日/每周/每月/不少于X次；发现问题后的处置与验证必须写清执行主体、完成时限、判定依据与留存的记录名称。禁止只写“加强、落实、确保”式无责任、无标准、无频次的空话。',
  // ── 4.60 I2-c 断源（用户实测指出：130 处同一句式骨架、语料 832 句中 42 句以「复查销项」收尾）──
  //
  // 旧文案（本行 + 下方变体池）是**句式模板 + 密度配额 + 词表**三件套：
  //   ①「全文每 1500 字至少 1 段完整闭环句式…整改闭环（整改/复查/销项/复验）三要素」→ 密度配额；
  //   ②「必须用至少 5 种不同的句式结构表达，**例如**：①…②…③…④…⑤…」→ 5 条**现成句式**，
  //      且 5 条的尾巴全是「复查销项/复查确认/验收销项/复核后销项/复查验收」；
  //   ③ 词面三要素判定（FIVE_ELEMENT_WORD_RES.acceptance）要求 整改|复查|销项 落在同一段。
  // 模型是**忠实执行**的：配额要求它每 1500 字补一段，模板告诉它怎么写，词表告诉它必须出现哪三个字
  // —— 三者叠加的确定性结果就是全文 40+ 条同尾句。G 线当年已发现「密度配额 × 句式不重复」在数学上
  // 不可满足，但解法是**再给一批模板**（5 条变体池），等于把病根加深了一层：可满足性买到了，
  // 代价是模板化从"碰运气"变成"必然"。
  //
  // 现口径（**断源**，不打补丁）：只要求闭环有实质，**不给句式、不给词表、不设密度配额**；
  // 词面判定侧同步放宽（acceptance 纳入验收/检测/记录类实质词，见 FIVE_ELEMENT_WORD_RES），
  // 使「写实质」与「得分」不再冲突——否则禁令只是把套话从一种换成另一种。
  '【闭环要有实质，不得以套语收句】管控措施的闭环必须写实质（谁执行、何时完成、依据什么判定、留下什么记录），**禁止**用“复查销项”“整改销项”“复查确认”“验收销项”“整改闭环”一类无执行主体、无依据、无记录的零信息尾词收句，也禁止将其作为措施段落的固定收尾；闭环要落到具体动作上（复测、送检、旁站、检验批验收、实测实量、资料归档等），不同段落各写各的，同一收尾表达全文不得超过 4 次。',
  '【空话禁用】措施句不得使用无动作对象、无量化标准的宣传口号式短语作为核心表述，不得以单字虚词（合理、充分、完善、切实、尽量、适时）作为措施句核心动词；一律改写为“责任岗位 + 执行动作 + 量化标准 + 检查频次”的可核查表述。',
...
export const FORMAL_WRITING_RULES = [
  ...
  // 事实来源分级两条已由 DOCUMENT_L0_COMMON_PREFIX 第 1/2 条承载（同义重复曾双重注入 ~210 字符）
  TENDER_BID_WRITING_RULES,
  // 4.17.3 示例数值泄漏根治（庐江/合肥师范双实锤：本节提示词曾写“按 45 日历天总工期倒排”“针对 4368m² 改造范围”
  // 作为格式示范，Writer 把 45 日历天整章照抄进正文并自洽展开进度体系——45 既非缓存串染也非检索串染，
  // 而是提示词示例数值被 LLM 当项目事实吸收）。示例一律去数值：只示范句式形态，具体数字必须来自绑定材料。
  ...
  // M4·写作三源规则：旧条目“计划类字段必须基于工程量/工期/定额自行推导”是写作层自算与蓝图锚点体系的核心矛盾源，
  // 替换为全局/章切片共用的三源规则模板（单点文案在 workflowRules.writingSpec.threeSourceRules）
  THREE_SOURCE_WRITE_RULES,
].join('\n');

export const SECTION_GENERATION_SAFETY_RULES = [
  '只生成当前节及其节内三级小节正文，不生成其他同级节，不重复章节一级标题。',
  // L0 重组（2.2）：事实分级已由 DOCUMENT_L0_COMMON_PREFIX 第 1 条承载，此处只保留小节级
  // 优先级语义；项目概况复述禁令已由 FORMAL_WRITING_RULES 承载（同义重复曾双重注入 ~130 字符）
  ...
  // WS1 结构标签禁令（写作侧单源文案；与检测 templatedLabelIssues / 修复 fixTemplatedLabels 同源口径）
...
/** Writer 系统前缀：统一公共段（L0 + FORMAL 完整规则）前置为开头，writer 身份句后移到规则之后固定位置。
 * （原 DOCUMENT_L0_SYSTEM_PREFIX legacy 回退已固化删除：统一前缀恒开，legacyPrefix 参数保留兼容调用点） */
export function writerSystemPrefix(): string {
 ...
 * （原 DOCUMENT_L0_SYSTEM_PREFIX legacy 回退已固化删除：统一前缀恒开，legacyPrefix 参数保留兼容调用点） */
export function docSystemPrefix(role: string): string {
...
function normalizeFormalChapterHeadings(markdown: string, chapters: Array<Pick<DocumentDraftChapter, 'title' | 'sections'>>) {
  ...
  const normalizeSectionHeading = (title: string, fallbackSourceSection?: string) => {
    ...
    // L5 降级合并重排：重复小节降级为当前小节的 H4，不消耗 H3 编号（sectionIndex 不递增）——
    // 历史缺陷：降级块占用编号后成稿 H3 出现 2.15/2.17/2.18/2.20/2.21/2.24 跳号
    //（降级块编号被后续归一转为 H4 不可见，剩余小节编号留下空档）；降级与编号重排必须同帧完成
...
export function finalizeDocumentMarkdown<T extends Pick<DocumentDraftChapter, 'title' | 'sections' | 'content'>>(markdown: string, chapters: T[], options: { forbidDrawingImages?: boolean; promptRules?: PromptDocumentRuleSet; bodyTableForbidden?: boolean; coverForbidden?: boolean } = {}) {
  ...
  const captionedMarkdown = options.bodyTableForbidden ? finalizedMarkdown : finalizeTableCaptions(finalizedMarkdown);
