...
export const MARKDOWN_TABLE_FORMAT_RULES = [
  ...
  '表格数据完整性硬约束：数据行每一列都必须有具体数据值，不得出现空单元格；不得用“—/若干/约/待定/暂无/不适用”等占位或模糊表达代替具体数据（合计/小计/总计/累计行的“—”不适用语义除外；规格型号列中无规格型号的小型机具如蛙式打夯机、小型夯实机可写“—”表示机具无型号，同属不适用语义）。数据优先取自本项目资料，资料未直接给出时按工程量、工期与专业定额工效推算具体数值，不得凭空编造型号。',
