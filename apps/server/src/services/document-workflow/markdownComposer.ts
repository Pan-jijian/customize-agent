...
export const TENDER_BID_WRITING_RULES = [
  ...
  // G 线 P1-12 量纲对齐：原规则同时要求「每 1500 字 1 段闭环句式」与「同一句式全文不得反复使用」——
  // 14 万字即约 93 段闭环句，而结构帧只有有限几种，「全文不反复」在数学上不可满足；检测端又按
  // 结构帧计数（sentencePatternThreshold = max(6, 正文字数/5000)，14 万字时=28）判模板化，
  // 于是「写得越合规越容易被判模板化」，正文质量在一个自锁区间里震荡。
  // 现口径：改为**可满足且与检测同源**的变体池要求 —— 句式结构不少于 5 种，单一结构帧占比不超过
  // 闭环句式总数的 1/5，使单帧命中数稳定低于检测阈值。
  '【闭环句式变体池（硬约束）】上述闭环三要素必须用**至少 5 种不同的句式结构**表达，例如：①「由X岗位每Y检查Z，发现问题即时整改并复查销项」；②「X工序由X岗位负责，每日不少于N次检查，不合格返工后由X复查确认」；③「实行X岗位每日巡查制度，问题登记后限期整改、由X验收销项」；④「X岗位每周检查并留存记录，整改结果经X复核后销项」；⑤「对X项由X岗位每月专项检查，问题整改完成后复查验收」。**同一句式结构的使用次数不得超过全文闭环句式总数的五分之一**（句模复读检测按结构帧计数，单一结构帧超阈值即判模板化并扣分）；相邻小节不得以同一句式开头。',
