...
import { qingtianReviewValidationIssues, runFullDimensionReview, splitChaptersIntoReviewBlocks } from './fullDimensionReview';
import { qingtianBlockReviewPrompt } from './qingtianReviewSpec';
import { buildExportGate } from './qualityValidation';
import type { DocumentDraftChapter, DocumentFactsModel, DocumentGenerationDiagnostics, DocumentTemplate, DocumentTemplateChapter } from './types';

vi.mock('./llmClient', () => ({ callDocumentLlmJson: vi.fn() }));
vi.mock('./rolePipeline', () => ({ repairChapterByQuality: vi.fn() }));
vi.mock('./documentGeneratorHelpers', () => ({ finalizeChapterContentQuality: vi.fn((content: string) => content) }));
...
describe('runFullDimensionReview', () => {
  ...
  it('高风险问题 → 定向修复 + 同块复评，章节内容被替换', async () => {
    ...
    repairMock.mockResolvedValue({ content: '修复后正文', appliedCount: 2, repairType: 'quality' as never });
  ...
  it('复评仍检出高风险 → 计入剩余清单', async () => {
    ...
    llmMock
      ...
      .mockResolvedValueOnce({ issues: [issue], templatingLevel: '无' });
    repairMock.mockResolvedValue({ content: '章节新内容（未真正修复工期矛盾）', appliedCount: 1, repairType: 'quality' as never });
