...
export function isValidProjectBasicFactValue(fieldId: string | undefined, rawValue: unknown) {
  ...
  // 4.55.12 巢湖实测：答疑澄清表格残片混入信息表（工期单元格值 =「现澄清为如下：条款号条款号条款名称
  // 条款名称编列内容编列内容1.3.2计划工期计划开工日期：2026年10月10日（具体开工日期以」）——表头格
  // 重复拼接 + 嵌套字段标签粘连，此前因含「2026年」被判满足工期句式而放行。澄清表残片整类拒收。
  if (/条款号|条款名称|编列内容|澄清为如下|现澄清|答疑文件|补遗书|投标人须知前附表/u.test(value)) return false;
  // 丰乐镇第五轮加固：计划工期值尾部编号粘连（「90日历天2.9招标范围」PDF 编号串行残留）拒收，
  // 与 scoreFactCandidate 拒收同口径——信息表不再自我复制固化脏值
  // 巢湖加固：值以嵌套字段标签（「计划开工日期：…」）开头时为表格串格残片而非工期口径，拒收
  if (fieldId === 'schedule_requirement') return value.length <= 90 && /\d+(?:\.\d+)?\s*(?:日历天|天|个月|月|年)/u.test(value) && !/^(?:计划)?开工日期\s*[:：]/u.test(value) && !/(?<=[日天])\s*[0-9]+(?:\.[0-9]+)?(?=[\s]*(?:招标|建设|质量|合同|项目|工程|资金|投标|开标|评标|付款|工期|开工|计划|计价|现场|资质|标段|[，,。；;]|$))/u.test(value);
