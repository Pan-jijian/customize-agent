...
export function hasProcessSequenceExpression(text: string): boolean {
  ...
  // 4.59 C1 顿号链（巢湖 ea380252 1.18.3 逐字实测）：「屋面及防水工程按基层清理、找坡找平、防水层施工、
  // 蓄水试验组织」是工序顺序的自然成文形态（「按 A、B、C、D…组织」= 按序推进）；旧词表只认「按…顺序」
  // 字面与箭头/编号/列表形态，该块被判 process=false 并计入「6 个分项方案内容要素不全」——逐字复核该句
  // 即工序声明，属误报（与写作侧「呈现形式不限」口径同向）。
  // 判据边界（防把清单枚举当工序）：① 必须由「按」引导；② 链上 ≥3 个环节；③ 链末必须落到顺序语义词
  //（顺序/先后/步骤/流程/组织/推进/安排/施工程序）——「按厂区道路、围墙、排水等配套内容施工」式
  // 范围/材料枚举 + 施工动词不判，避免枚举句冒充工序。
  if (/按[^。；\n]{2,40}(?:、[^。；\n]{2,40}){2,}[^。；\n]{0,8}(?:顺序|先后|步骤|流程|组织|推进|安排|施工程序|作业程序)/u.test(text)) return true;
  return false;
}
...
export function workPackageContentElementFlags(block: string): { scope: boolean; process: boolean; method: boolean } {
  ...
  // 4.59 C1 scope 补漏（巢湖 cfb0a0da 1.4.3 逐字实测）：「室外附属工程包括室外道路、围墙、排水、给水、
  // 电气及路灯等配套内容」被判 scope=false——「作业对象」不止「数值+单位」一种形态，
  // 「包括/涵盖/覆盖…等（内容|项目|工程|分项|专业|部位）」是范围声明的自然句面（与「施工范围：」标签同族）
  const scope = /(?:施工)?(?:概况|范围)[:：]\s*\S|工程量|作业对象|部位|总量|共\s*\d|\d+(?:\.\d+)?\s*(?:km|㎡|m²|m2|m3|m³|座|套|处|盏|株|棵|延米|吨)(?![\dA-Za-z])|\d+(?:\.\d+)?\s*m(?![\dA-Za-z])|(?:包括|涵盖|覆盖|含)[^。；\n]{2,60}等[^。；\n]{0,10}(?:内容|项目|工程|分项|专业|部位)/u.test(block);
  ...
  // 4.59 C1 method 补漏（巢湖 ea380252 1.19.3 逐字实测）：「防雷接地利用柱主筋与圈梁钢筋焊接171处，
  // 避雷引下线2969.52m，焊接搭接长度不小于6d，双面施焊」被判 method=false——焊接工艺参数
  //（搭接长度/双面施焊/满焊/焊缝/坡口）是施工方法的实质内容（与下方「工艺参数不足」建议文案里
  // 已列明的「搭接长度」同族），词表漏判致该块计入「6 个分项方案内容要素不全」
  const method = /(?:施工)?方法(?:[:：]\s*\S|(?:采用|为|按|以|包括|如下))|工艺参数|验收标准|检测|试验|记录|检查(?!井)|巡查|检验|整改|复查|销项|养护|台账|核对|复核|计量|MU\s*\d+|C\d+|抗压强度|抗折强度|强度等级|配合比|坍落度|工序卡|操作规程|作业指导书|技术交底|搭接长度|双面施焊|三面施焊|满焊|焊缝|焊脚|坡口|焊接工艺参数/u.test(block);
...
export function normalizeSubsectionTitleForDedup(title: string): string {
  return title
     ...
     *
     * 4.58 R1 实测修：**单段编号后必须跟分隔符**，否则不剥。
     *
     * 旧判据以 `^\d+(?:\.\d+)*[.．]?` 开头、以 `\s*` 收尾（允许零空白），于是把单体编号当成小节号剥掉：
     * 「`1#厂房土建专项工程`」→「`#厂房土建专项`」，而正文 H3「`1.22 1#厂房土建专项工程`」
     * 先剥 `1.22 ` 得「`1#厂房土建专项`」——**两侧不等**，导致
     * **每一个以「N#」开头的小节都被判「规划小节未落位」**。
     * 实测巢湖 `主要施工方法与技术措施` 章 14 条未落位告警里，5 条（`1#厂房土建结构与基础工程`、
     * `1#厂房土建专项工程`、`1#厂房土建装饰装修工程`、`1#厂房土建天棚工程`、`1#厂房安装专项工程`）
     * 全是这个假报；该章正文里 `### 1.22 1#厂房土建专项工程` 等标题**都存在且带正文**。
     *
     * 判据：**多段编号**（含点号，如 `1.2`）视为小节号直接剥；
     * **单段编号**（裸 `1`）仅在后跟分隔符（空白 / `.．、)）`）时才剥——`1#`、`2#` 是单体编号而非小节号。
     */
    .replace(/^\d+(?:\.\d+)+(?![#＃])\s*[.．、)）]?\s*/u, '')
    .replace(/^\d+(?=[\s.．、)）])\s*[.．、)）]?\s*/u, '')
    .replace(/[（(][^（）()]*[）)]/gu, '')
...
/**
 * 评标否决/废标规则条款判定（4.28.x 舒城实测：「技术文件明显文不对题…一律否决其投标」条款
