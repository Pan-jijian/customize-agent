...
export function buildGenerationBudget(input: {
  ...
  // 并发预算：超长显式小节章优先逐章产出 checkpoint；长章保守、短章提速；资料稀疏保守
  ...
  // 审查流水线并发：审查/修复是重 LLM 调用，fast 串行、其余 2 路并行，避免与生成争抢模型并发
  const reviewConcurrency = strategy.mode === 'fast' ? 1 : 2;
  ...
  triggers.push(`章节并发 ${chapterConcurrency}/${chapterCount}，审查流水线并发 ${reviewConcurrency}，每章证据预算 ${Math.round(floorChars / 1000)}k-${Math.round(ceilingChars / 1000)}k 字符，修复轮次预算 ${repairRoundBudget}`);
  return { strategy, chapterConcurrency, reviewConcurrency, evidenceFloorChars: floorChars, evidenceCeilingChars: ceilingChars, repairRoundBudget, triggers };
}
