...
const INTERNAL_TERM_ANCHORS: Array<{ anchor: string; required: RegExp }> = [
  ...
  // 真实生成回归（合肥师范 4.12.8）：「本专业工程主要清单项落位如下」「技术要求进行专项落位」——
  // 「落位」是生成系统任务术语，正式正文零合法用途；数据一致性修复轮把「峰值口径」要求写入正文
  { anchor: '本专业工程主要清单项落位如下', required: /清单项落位|落位如下/u },
  { anchor: '针对招标文件及补疑文件的技术要求进行专项落位', required: /专项落位/u },
  { anchor: '全文数据口径必须唯一', required: /数据口径|峰值口径|控制口径|口径必须唯一/u },
] as const;
