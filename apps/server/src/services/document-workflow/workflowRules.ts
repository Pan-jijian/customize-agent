...
export interface WorkflowRulesConfig {
  ...
  writingSpec: {
    ...
    /** 分部分项专项验收阈值 */
    divisionQuality: { blockerMinPackages: number; minPackages: number; minParamsPerPackage: number; minArrowChainLength: number; minPackageChars: number; balanceRatio: number };
...
export const DEFAULT_WORKFLOW_RULES: WorkflowRulesConfig = {
  ...
  writingSpec: {
    ...
    divisionQuality: { blockerMinPackages: 3, minPackages: 5, minParamsPerPackage: 4, minArrowChainLength: 4, minPackageChars: 150, balanceRatio: 1 / 3 },
    writeRules: {
      ...
      division: '【主要分部分项工程施工方案专项要求】每个“#### 分项工程方案”三级小节必须包含施工概况（本项目作业对象、部位、工程量）、工艺流程（用“→”串联关键工序）和施工方法（工具机具、材料规格、验收标准）。严禁用“**分项名**”粗体行代替“#### 分项名”小节标题，也不得把多个分项合并写在一个段落里。施工方法叙述中的连续工序序列同样用“→”串联（如“基层清理→放线定位→分层摊铺→碾压→压实度检测→验收”），每个分项方案的方法段正文至少 1 条不少于 4 个环节的箭头工序链，不得只把箭头局限在工艺流程行；每个分项方案正文必须落位至少 4 个工艺参数（mm、MPa、间距、偏差、坡度、养护天数、试验压力、搭接长度等），参数来自绑定材料或行业通用规范值，不得编造；纯设备配置型小节必须写型号、规格、容量、数量参数；不得写“按规范施工”“结合实际执行”式空话。分项间深度必须均衡：门窗维修、立面修补、设备安装等小分项同样要写足施工概况、工艺流程、施工方法三段（每个分项不少于 150 字），不得一句话带过；严禁写“其他专业工序引用相应章节内容”“详见相关章节”等自我消解语，工艺流程只能在本分项方案内展开，不得另行拆节复述。',
    },
  },
};
