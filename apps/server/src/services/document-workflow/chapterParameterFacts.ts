 ...
 */
const PARAMETER_BARE_QUANTITY_RE = /^\d+(?:\.\d+)?\s*(?:%|元|万元|亿元|项|个|天|月|年|周|小时|分钟|人|次|份|批|层)$|^\d{4}[.\-/]\d{1,2}$/u;
...
const PARAMETER_PROSE_RE = /[。！？]|[：:]\s*[\d（(]|(?:应|须|不得|必须|严禁|宜)[^，。；]{0,20}(?:采用|符合|按照|满足|设置|办理|执行|组织|进行|大于|小于|超过|支付|承担|参加|具备|具有|提供|建立|完成|取得|达到|构成|包括|包含|纳入|列入|签署|加盖)|[^，。；]{6,}为[^，。；]{6,}/u;
