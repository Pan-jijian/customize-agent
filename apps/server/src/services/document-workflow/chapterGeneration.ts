...
export async function buildSectionFactCard(sectionTitle: string, evidence: DocumentEvidence[], embedDocuments?: (texts: string[]) => Promise<number[][]>): Promise<SectionFactCard> {
  ...
  /**
   * 【当前小节写作任务卡】——把**终检判据**编译为**写作期可执行约束**（4.58 P2：R2 三要素 + R3 数值来源）。
   *
   * ## 为什么要与终检判据逐字对齐
   *
   * 4.58 实测：一轮真实生成的 30 条阻断里 **29 条（96.7%）是终检才发现**，写作期无对应落点。
   * 其中「分部分项方案要素不全/缺工序顺序/工艺参数不足/正文过短」「未溯源数值」「无主数值」
   * 等族的判据早已存在（`workPackageContentElementFlags` 的三要素判定 `scope/process/method`、
   * `factReconciliation` 的无据不写），只是**从未作为写作期约束下发**——写手不知道会被查什么。
   *
   * 故本任务卡的要素清单与判据**逐项同源**：
   * ① 作业对象与工程量 ↔ `workPackageContentElementFlags().scope`
   * ② 工序顺序        ↔ `.process`（复用 `hasProcessSequenceExpression` 的形态不限口径）
   * ③ 施工方法        ↔ `.method`（含工艺参数条数要求）
   * ④ 数值来源        ↔ `factReconciliation`「无据不写」 + 危大实参要求
   *
