...
export function displayChapterTitle(title: string) {
  ...
  while (cleaned && cleaned !== prev) {
    ...
    cleaned = cleaned
      ...
      // 多段编号整段剥离（4.40 残号根治）：旧式 ^\d+(?:\.\d+)*[、.．\s]+ 在「2.16杭北干渠…」上
      // 回退匹配「2.」留下「16杭北…」残号（清单/图纸条目编号 2.16/4.2/4.5/4.6 随名称进入标题，
      // 成稿出现「2.1.1 16杭北干渠…」）；多段编号必须整段消费；紧跟计量单位（万/米/吨…或拉丁字母）
      // 的多段数字是标题正文（「3.5米宽临时便道」）不剥
      .replace(/^\d+(?:[.．]\d+)+(?![万亿千百十米㎡吨元A-Za-z\d])\s*[、.．]?\s*/u, '')
      // 单段编号必须后随分隔符或空白；点号后紧跟数字属多段编号（交给上行整段剥离），不在此回退剥半段
      .replace(/^\d+(?:[、，,]\s*|\s+|\.\s*(?![0-9])|．\s*(?![0-9]))/u, '')
