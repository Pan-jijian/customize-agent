...
function instructionFor(draftChapter: DocumentDraftChapter, deficits: ControlLoopChainDeficit[], budgetEntry?: ChapterBudgetEntry): string {
  ...
  for (const deficit of deficits) {
    lines.push(`- ${deficit.label}（缺：${deficit.missing.join('、')}）：${deficit.prompt}请在本章对应小节内把缺失环节**按其业务实质**补写清楚。`);
  }
  lines.push(
    '修复要求：',
    '1. 只做局部修改：在对应小节内扩写补实，或自然融入一段补写文字；不得新增、删除或合并小节，不得改动无关内容；',
    '2. 补写须结合本章既有语境（本工程工序、责任岗位、检查频次、记录方式），写清每个缺失环节“谁执行、何时执行、依据什么判定、留存什么记录”，与既有句子自然衔接；',
    // 4.60 I2-c 断源：旧第 3 条要求「必须出现缺失要素的**标准词面**，不得以近义改述代替」——
    // 该条把「补环节」退化成「补字符串」，是正文被反复写回同一串套语的直接执行端（连同上方的
    // “A—B—C”成链文案）。现改为：按实质补，且**不得**用套语尾巴充数（与写作侧禁令同口径）。
    '3. 补写的是**环节本身**（谁、何时、依据、记录），不是环节的名称：不要为了出现某个词而加尾巴，禁止以“复查销项”“整改销项”“复查确认”“验收销项”一类无信息尾词收句；同一收尾表达全文不得超过 4 次；',
