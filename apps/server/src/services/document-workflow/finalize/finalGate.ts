...
export async function stageFinalGate(session: FinalizeSession): Promise<void> {
  ...
  // C1 挂起清单（批 1）：门禁失败时构建结构化精准人工清单（分类/定位/问题/建议/修复路径/检测器），
  // details 逐条带修复路径追溯（替代裸 message 列表）；清单随 reviewMetadata 归档、warningIssues 置顶
  ...
  session.finalStages.push(displayStage({ type: 'validation', roleId: 'agent-final-gate', status: session.qualityBundle.finalExportGate.passed ? 'success' : 'failed', message: session.qualityBundle.finalExportGate.passed ? 'Agent 最终门禁通过' : `Agent 最终门禁阻断 ${blockingCount} 个问题（已显式挂起，人工处理清单见明细）`, details: session.suspensionChecklist ? formatSuspensionDetails(session.suspensionChecklist) : [] }, { subtitle: 'Agent 最终门禁' }));
