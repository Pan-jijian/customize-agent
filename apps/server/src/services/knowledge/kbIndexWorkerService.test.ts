...
vi.mock('./kbService', () => ({ getMultiProjectManager: vi.fn() }));
vi.mock('./kbOperationLog', () => ({ upsertKbOperation: vi.fn() }));
vi.mock('../document-workflow/projectIntelligence', () => ({ startProjectIntelligenceBuild: vi.fn() }));

import { fork } from 'child_process';
import { runIndexLoop } from '@customize-agent/knowledge';
import { getMultiProjectManager } from './kbService';
import { upsertKbOperation } from './kbOperationLog';
