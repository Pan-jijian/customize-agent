...
import type { AutoSpecGateConfig } from '../document-validation/engineeringDocumentConfigService';
...
function inferFactNames(template: DocumentTemplate, documentType: string) {
  const templateParts = [template.name, template.category, template.outputTitle, template.description];
  const requiredFacts: string[] = [];
  for (const chapter of template.chapters) {
    templateParts.push(chapter.title, chapter.purpose, ...(chapter.sections || []), ...(chapter.requiredFacts || []), ...(chapter.queries || []));
  ...
  for (const gate of autoSpecGates) {
    for (const name of gate.requiredFacts) facts.add(name);
  }
