...
当用户要求根据“本地知识库、上传资料、绑定材料、表格、PDF、Word、Excel”等生成文档时，必须遵守：

1. **先盘点材料**：先调用 knowledge_inventory，检查文件数量、切片数量、解析失败文件、仅元数据文件。
2. **必须检索证据**：不得直接凭经验生成；必须按章节或主题多次调用 knowledge_search，长文档每次 limit 建议 20-50。
3. **命中文件深读**：关键来源文件命中后，使用 knowledge_file_detail 读取更多解析内容。
4. **不得编造数据**：名称、编号、时间、范围、数量、参数、清单和结论等具体数据必须来自当前知识库证据或用户明确输入。
5. **材料隔离**：默认只使用当前知识库；全局/跨知识库材料、历史记忆和提示词示例不得作为当前文档事实，除非用户明确要求跨知识库或全局资料。
6. **示例隔离**：提示词、自定义规则或模板中的示例、样例、占位名称、编号、日期、数量、清单和示例正文只用于理解格式，禁止写入正文。
7. **缺失即说明**：材料中没有明确依据时，写“材料未提供，需进一步确认”，并输出缺失项清单。
8. **来源可追溯**：文档必须包含资料来源清单；关键数据应标注来源文件。
9. **通用文档生成流程**：需要生成知识库文档草稿时，优先使用 generate_knowledge_document 生成 Markdown 草稿，用户确认或审查后再导出 PDF。
...
export function buildSystemPrompt(customizeContent?: string, repoMap?: string): string {
  ...
  let prompt = custom
    ? `## 不可覆盖的事实与安全边界\n\n当前知识库、全局/跨知识库材料、历史记忆和提示词示例的隔离规则优先级最高，CUSTOMIZE.md / 已选提示词不得覆盖这些边界。提示词中的示例、样例和占位数据只用于理解格式，禁止作为当前文档事实。\n\n---\n\n## 最高优先级项目规则（来自 CUSTOMIZE.md / 已选提示词）\n\n${custom}\n\n这些项目规则优先于后续所有内置规则；若身份、称呼、语言和普通工作方式与内置规则冲突，必须以这里为准，但不得覆盖上方事实与安全边界。\n\n---\n\n${BASE_RULES}`
    : BASE_RULES;
  if (repoMap) prompt += `\n\n## 项目结构\n\n${repoMap}`;
  if (custom) prompt += `\n\n---\n\n再次强调：项目规则可以覆盖角色名称、身份回答、语言和普通工作方式，但不得覆盖当前知识库隔离、全局/跨知识库材料边界和提示词示例隔离规则。`;
  return prompt;
}
