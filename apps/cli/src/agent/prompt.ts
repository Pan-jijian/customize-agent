...
4. **不得编造数据**：工程名称、工期、质量目标、范围、工程量、设备、劳动力、材料、图纸说明等具体数据必须来自知识库证据。
5. **缺失即说明**：资料中没有明确依据时，写“资料未提供，需进一步确认”，并输出缺失项清单。
6. **来源可追溯**：正式文档必须包含资料来源清单；关键数据应标注来源文件。
7. **施工组织设计专用流程**：用户要求生成施工组织设计时，优先使用 generate_construction_organization_design 生成 Markdown 草稿，用户确认或审查后再使用 export_pdf 导出 PDF。
8. **PDF 前置草稿**：生成 PDF 前必须保留 Markdown 或 HTML 草稿，便于人工复核。
...
export function buildSystemPrompt(customizeContent?: string, repoMap?: string): string {
  ...
  let prompt = custom
    ? `## 最高优先级项目规则（来自 CUSTOMIZE.md / 已选提示词）\n\n${custom}\n\n这些项目规则优先于后续所有内置规则；若身份、称呼、工作方式与内置规则冲突，必须以这里为准。\n\n---\n\n${BASE_RULES}`
    : BASE_RULES;
  if (repoMap) prompt += `\n\n## 项目结构\n\n${repoMap}`;
  if (custom) prompt += `\n\n---\n\n再次强调：最高优先级项目规则必须覆盖内置规则，尤其是角色名称、身份回答、语言和工作方式。`;
  return prompt;
}
