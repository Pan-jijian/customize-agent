...
- 修复"导出门禁未通过：跨章一致性冲突（建设规模数值不一致）"误报与死循环：

  - 检测侧与裁决侧口径收窄：用地面积/占地面积是独立字段，不再与建设规模（建筑总量）混比；正文正确转述资料"总用地面积 X㎡"不再被误判为冲突（此前会阻断导出，且"继续生成"全量重跑必然复现，按钮永远失败）
  - 子项口径隔离：门卫室/配电房等具体建筑物建筑面积（子项分层）不再判为总量冲突
  - 确定性冲突（数值口径、工序规格）并入生成流程内全局一致性修复闭环：真实冲突在导出前自动修复，不再只靠用户手动点击"继续生成"
...
- Improve document workflow generation by moving explicit outline subsection planning to LLM-driven preplanning and adding detailed dynamic workflow progress display.
- Updated dependencies
  - @customize-agent/server@4.0.71
