...
import { Card, Button, Tag, Modal, Input, Select, Spin, Row, Col, Space, Popconfirm, Form, App } from 'antd';
...
export default function ModelsPage() {
  ...
  const openEdit = async (p: ProviderInfo) => {
    ...
    setEditApiKey('');
    // 从 API 获取完整 Provider 数据，确保 baseUrl 等字段正确回显
    try {
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      await saveProvider(editModelName.trim(), { apiKey: editApiKey || undefined, baseUrl: editBaseUrl || undefined, protocol: editProtocol });
  ...
  return (
    <div className="space-y-6 animateFadeIn">
      ...
      <Card title={t('models.modelList')} size="small">
      ...
      <Modal title={t('models.editModel')} open={editOpen} onCancel={() => { setEditOpen(false); setEditTarget(''); setEditModelName(''); setEditApiKey(''); setEditBaseUrl(''); setEditProtocol('openai'); }} onOk={() => { void handleEdit(); }} confirmLoading={editSaving}>
        <Form layout="vertical" size="medium">
          ...
          <Form.Item label={t('models.apiKey')} help={t('models.apiKeyEditHint')}><Input.Password value={editApiKey} onChange={(e) => setEditApiKey(e.target.value)} placeholder={t('models.apiKeyEditPlaceholder')} /></Form.Item>
