  ...
  return (
    <div className="space-y-6 animateFadeIn">
      <div className="flex items-center justify-between">
        ...
        <Button icon={<ReloadOutlined />} onClick={() => { void load(); }}>{t('common.retry')}</Button>
      </div>
      ...
      <Card title={t('overview.quickActions')} size="small">
        <Space wrap>
          <Button onClick={() => router.push('/knowledge/manage')}>{t('overview.goToKnowledge')}</Button>
          <Button onClick={() => router.push('/models')}>{t('overview.goToModels')}</Button>
          <Button onClick={() => router.push('/settings')}>{t('overview.goToSettings')}</Button>
          <Button onClick={() => router.push('/prompt')}>{t('nav.promptManagement')}</Button>
          <Button onClick={() => router.push('/context/long-term')}>{t('nav.contextEngineering')}</Button>
        </Space>
      </Card>
    </div>
  );
